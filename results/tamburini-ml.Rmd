...
# Format data
plt_typs <- c("cLEC", "Collecting", "fLEC", "Ptx3_LEC", "BEC")
...
# Format model data
mods        <- rf_mods_best$mod
names(mods) <- rf_mods_best$subtype

f1_scores <- rf_mods_best %>%
  filter(subtype != "BEC") %>%
...
plt_dat <- f1_scores %>%
  filter(mouse != "6wk-3wk") %>%
  dplyr::select(F1, `Balanced Accuracy`, mouse, subtype, mod_type) %>%
  mutate(
    subtype  = fct_relevel(subtype, rev(lvls)),
    mod_type = fct_relevel(mod_type, lvls),
    mouse    = fct_relevel(mouse, mice),
    # lab      = as.character(round(F1, 2))
    lab      = as.character(round(`Balanced Accuracy`, 2))
  )

plt_dat %>%
  ggplot(aes(mod_type, subtype, fill = `Balanced Accuracy`)) + 
  geom_tile(color = "white") +
...
plt_typs %>%
  walk(~ {
    ...
    clrs <- rev(pred_clrs)
    clrs[c("Ag-low", "Ag-competent")] <- "#56B4E9"
    clrs["other"] <- "white"
    ...
    u_so <- lec_so %>%
      mutate_meta(
        ...
        pred_grp1 = ifelse(pred_grp %in% c("Ag-high", "Ag-low"), pred_grp, "other"),
        pred_grp2 = ifelse(pred_grp %in% c("Ag-high", "Ag-competent"), pred_grp, "other")
      )
    ...
    ag_high_u <- mice[mice %in% lec_so$mouse] %>%
      ...
      wrap_plots(nrow = 1) +
      plot_annotation(
        title = "Predicted Ag-competent vs Ag-high",
        theme = theme(
          plot.title = element_text(hjust = 0.5, size = ttl_pt2)
        )
      )
