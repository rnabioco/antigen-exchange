...
```{r "Ag signal broad cell types", fig.width = 9}
...
Ag signal is shown below for LEC subsets.

```{r "Ag signal LEC subsets", fig.width = 9, fig.height = 6}
# Format data for plotting
dat <- lec_df %>%
  filter(mouse != "6wk-3wk")
...
```{r "F1 scores 2", fig.width = 8, fig.height = 3.5}
f1_scores %>%
  filter(
    mod_type == subtype,
    mouse == "d14"
