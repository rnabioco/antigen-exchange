...
train_rf <- function(df_in, data_clmn, param_lst, sample_size = 135,
  ...
  if (balance_groups) {
    ...
    # classes <- as.character(unique(args$data[[data_clmn]]))
    # 
    # if (all(classes %in% names(args))) {
    #   class_wts <- set_names(as.numeric(args[classes]), classes)
    # 
    #   args$probability   <- TRUE
    #   args$class.weights <- class_wts
    #   args[classes]      <- NULL
    # }
    # 
    # browser()
  ...
  res <- pmap_dfr(rf_params, ~ {
    ...
    # SETTING CLASS WEIGHTS DOES NOT IMPROVE ACCURACY OF CLASS BALANCE, WHY???
    # Check for class.weights parameter
    # case.weights <- vector("numeric", length(cases))
    # case.weights[cases == "high"] <- args$high_wts
    # case.weights[cases == "low"]  <- args$low_wts
    # args$case.weights             <- case.weights
    # args$high_wts <- args$low_wts <- NULL
    # classes <- as.character(unique(args$data[[data_clmn]]))
    # if (all(classes %in% names(args))) {
    #   class_wts <- set_names(as.numeric(args[classes]), classes)
    # 
    ...
    prds <- mod %>%
      ...
      rename_with(.cols = everything(), ~ str_c(.x, "_accuracy"))
    ...
    cli::cli_alert_info("{args$n} / {n_params}")
...
rf_mods <- typs %>%
  map_dfr(~ {
    ...
    param_lst <- list(
      ...
      # num.trees     = seq(50, 800, 100),
      # low           = c(1, 5, 10, 20, 50, 100),
      # high          = 1,
...
lec_so <- lec_so %>%
  AddMetaData(rf_preds, col.name = c("rf_pred", "rf_correct"))
