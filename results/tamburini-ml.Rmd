...
rf_predict <- function(mod, dat, data_clmn = NULL, return_stats = TRUE,
  ...
  # Calculate F1 score
  f1_score <- caret::confusionMatrix(
    data = res$pred,
    reference = res[[data_clmn]],
    positive = "high"
  )
  
  f1_score <- as_tibble(as.list(f1_score$byClass))
...
rf_mods <- rf_stats %>%
  filter(
    frac_high_mean > target_frac - 0.1,
    frac_high_mean < target_frac + 0.1,
  ) %>%
  arrange(
    desc(F1),
    abs(frac_high_mean - target_frac)
  ) %>%
  group_by(subtype) %>%
  dplyr::slice(1:15) %>%
  ungroup() %>%
...
best_mods <- rf_mods %>%
  filter(high_accuracy > 0.75, low_accuracy > 0.75) %>%
...
# Predict Ag class
pred_dat <- lec_so %>%
  FetchData(unique(c(rf_dat_clmn, all_feats))) %>%
  as_tibble(rownames = ".cell_id") %>%
  rename_with(~ str_replace_all(.x, "-", "_")) %>%
  rename_with(~ str_replace_all(.x, "\\(|\\)", "_")) %>%
  rename_with(~ str_c("x", .x), matches("^[0-9]")) %>%
...
lec_so <- lec_so %>%
  mutate_meta(
    ...
    training = .cell_id %in% best_mods$training_bcs[[1]]
  )
...
modules %>%
  imap(~ {
    lec_so %>%
      FetchData(c("subtype", "mouse", "pred_grp", "training", .y)) %>%
      filter(
        subtype %in% plt_typs,
        !training
      ) %>%
