...
naive_fig <- fig_dat %>%
  ...
  imap(~ {
    .x %>%
      ...
      geom_boxplot(outlier.size = 0.25, width = 0.6) +
      facet_grid(module_type ~ ., scales = "free_y") +
      scale_fill_manual(values = lec_clrs) +
...
```{r "FIG 2 CSHL POSTER", fig.width = 12, fig.height = 7}
bot <- plot_grid(
...
# Plot example genes
gns <- list(c("Psap", "Grn"), c("Ctsd", "Lgmn"), c("Prcp", "Atp6v1a"))
...
```{r "FIG 3 CSHL POSTER", fig.width = 11, fig.height = 11}
top <- plot_grid(
  rf_bar_1, f1_bars,
  nrow  = 1,
  align = "h",
...
```{r "FIG 4 CSHL POSTER", fig.width = 11.5, fig.height = 12}
...
```{r "Cormac's figure"}
img <- readPNG(here("results/figures/cshl_fig6c.png"))
img <- rasterGrob(img, interpolate = TRUE)
...
plot_grid(
  chikv_ag_bars, chikv_ag_bxs, fig6c,
  nrow = 1,
  labels = c("A", "B", "C"),
  label_size = ttl_pt2 * 1.5
)
