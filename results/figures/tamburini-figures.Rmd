...
params:
  ...
  mod_dir:         "~/Dropbox/Ryan/Projects/antigen-exchange/results/models/2025-03-23"   # ML models
  ...
  table_dir:       "results/tables/2025-03-23"                                            # tables
...
- [x] &nbsp;Train additional models to further corroborate predictions
      ([R3C7](#R3C7)) and to improve human predictions ([R1C5](#R1C5))
- [x] &nbsp;Revise cell type annotations, check epithelial cells ([R3C3](#R3C3))
- [x] &nbsp;Modify LEC subset labels used for CHIKV prediction plots, need these
...
- [x] &nbsp;Check CCR7hi DC population for cDC1s ([R1C6](#R1C6)).
      They express Zbtb46, but very sparse expression of Xcr1, so no
      indication they are cDC1s.
- [x] &nbsp;Report Pearson correlation for Fig S4E-F ([R3C6](#R3C6)).
      Correlation coefficients and p values are included in source data
      spreadsheet.
- [x] &nbsp;Adjust [Figure 1] and [Figure S2] colors to make them more distinguishable
      ([R2-minor](#R2-minor)).
- [x] &nbsp;Generate source data tables for all figures.
...
# Experiments to show in main figures
exps <- sort(unique(c(lec_so$exp, dc_so$exp)))
...
# Source data to write to excel file
SOURCE_DATA <- list()
```
...
top_fov <- "D"
med_fov <- "B"  # LN section showing medulla
bad_fov <- "C"  # LN section from very outside edge
...
cd45neg_typs <- c("LEC", "BEC", "Fibroblasts")

typ_ag_dat <- all_meta %>%
  filter(
    vaccination == "single",   # the number of LECs is lower than what is shown
    cell_type != "unassigned"  # in S2A since this heatmap does not include the 
  ) %>%                        # dual immunized samples
  group_by(cell_sort) %>%
  mutate(
...
heat_dat <- typ_ag_dat %>%
  ...
  summarize(
    `Ag_score mean`   = mean(Ag_score),
    `Ag_score median` = sd(Ag_score),
    n = n(),
    .groups = "drop",
  ) %>%
  group_by(cell_type) %>%
  mutate(x_lab = str_c(cell_type, "\nn = ", label_comma()(sum(n)))) %>%
  ungroup() %>%
  mutate(
    tm    = fct_relevel(as.character(tm), rev(as.character(tms))),
    x_lab = fct_reorder(x_lab, `Ag_score mean`, mean, .desc = TRUE)
  )

# Create heatmap summarizing Ag scores
typ_heat <- heat_dat %>%
  ggplot(aes(x_lab, tm, fill = `Ag_score mean`)) +
  geom_tile() +
...
fig1_ag_u <- fig1_dat %>%
  imap(~ {
    ...
    .add_umap_labels(plt, arrows = FALSE)
  })

# Format source data
list(LEC = "fig1C-F", DC = "fig1G-J") %>%
  iwalk(~ {
    SOURCE_DATA[[.x]] <- fig1_dat[[.y]] %>%
      select(
        cell_id = .cell_id,
        subtype,
        UMAP_1 = hUMAP_1,
...
```{r "Fig 1 diagram"}
img <- here("results/figures/experiment_design/fig1_exp_design.png")
...
names(bot) <- LETTERS[3:(2 + length(bot))]
...
D.  The expression of key marker genes is shown for LEC subsets.
    The center line, box limits, whiskers, and points represent the median,
    interquartile range (IQR), the range within 1.5×IQR from the box limits,
    and points outside this range (outliers), respectively.
E.  The expression of key marker genes is shown for DC subsets as described in D.

<u>Notes</u>

* Both replicates are included in each of these plots.
...
typ_u <- typ_dat %>%
  plot_scatter(
    ...
    plot_lvls    = c("BEC", "LEC", "Mon/Mac"),
    size         = 0.001,
    n_label      = "legend"
  ) +
  geom_text(
    aes(-Inf, Inf, label = n, color = NULL),
    data = n_dat,
...
# Format source data
SOURCE_DATA$`fig1B,S2A` <- typ_dat %>%
  mutate(replicate = rep_labs[as.character(exp)]) %>%
  select(
    cell_id = .cell_id,
    CD45    = cell_sort,
    tm,
  ...
  imap(~ {
    ...
    .x %>%
      ...
      mutate(
        ...
        replicate   = rep_labs[as.character(exp)]
      )
  })
...
# Format source data
list(LEC = "fig1C-F,S2B,D", DC = "fig1G-J,S2C,E") %>%
  iwalk(~ {
    SOURCE_DATA[[.x]] <<- subtype_dat[[.y]] %>%
...
```{r "Fig S2", fig.width = 16, fig.height = 20}
figS2 <- typ_u /
...
B.  Ag-scores are shown for each timepoint for CD45+ cell types,
    as described in B.
C.  Ag-scores are shown for each timepoint for LEC subsets, as described in B.
D.  Ag-scores are shown for each timepoint for DC subsets, as described in B.
...
typ_ag_bxs <- dat %>%
  map(~ {
    .x %>%
      create_ag_boxes(
        clrs = typ_clrs,
        facet_vars = c("exp", "cell_type")
      )
  })

# Format source data
list(`CD45-` = "figS3B", `CD45+` = "figS3C") %>%
  iwalk(~ {
    SOURCE_DATA[[.x]] <<- dat[[.y]] %>%
      select(
        cell_id   = .cell_id,
...
# Format source data
list(LEC = "figS3D", DC = "figS3E") %>%
  iwalk(~ {
    SOURCE_DATA[[.x]] <<- bx_dat[[.y]] %>%
      select(
        cell_id   = .cell_id,
        replicate = exp,
        tm,
...
```{r "Fig S3", fig.width = 15, fig.height = 17.5}
top <- plot_grid(
...
figS3 %>%
  .save_fig("figS3.png", w = 15, h = 17.5)
...
B.  The fraction of cells predicted to be Ag-competent is shown for each LEC
    subset 14, 21, and 42 days post immunization.
C.  Ag-high module score is shown for Ag-low, Ag-high, and predicted
    ...
    correction.
D.  UMAP projections show cLEC Ag-high module scores for each timepoint (top)
    and Ag-high and Ag-low cLECs (bottom).
...
# Format source data
SOURCE_DATA$fig2A <- plt_dat %>%
  as_tibble(rownames = "cell_id") %>%
  mutate(Ag_class = str_c("Ag-", !!sym(rf_dat_clmn))) %>%
  select(
    cell_id,
    tm,
    cell_type = subtype,
...
rf_bar_1 <- rf_bar_dat %>%
  ...
  labs(y = "fraction of cells", x = "days post immunization") +
  bar_thm +
  theme(
    legend.title = element_blank(),
    legend.position = "bottom"
...
# Format source data
SOURCE_DATA$`fig2B,S5A` <- rf_bar_dat %>%
  as_tibble(rownames = "cell_id") %>%
  select(
    cell_id,
    tm,
    cell_type = subtype,
    Ag_score,
    Ag_predication = ml_pred_1,
    Ag_class       = ml_pred_1_grp,
    correct_prediction = ml_pred_1_correct
  )
```

```{r "ML module boxplots", fig.width = 10, fig.height = 4}
# Format plot data
# * exclude dual immunization data
clmns <- c(
...
# Format source data
SOURCE_DATA$`fig2C,S5B` <- rf_bx_dat %>%
  rename_with(.cols = matches("_(low|high)$"), ~ {
    .x %>%
      str_replace("_low", " Ag-low module") %>%
      str_replace("_high", " Ag-high module")
  }) %>%
  select(
    cell_id = .cell_id,
    tm,
...
u_dat <- lec_so@meta.data %>%
  as_tibble(rownames = ".cell_id") %>%
  filter(tm %in% rf_tms) %>%
  mutate(!!sym(rf_dat_clmn) := str_c("Ag-", !!sym(rf_dat_clmn)))
...
# Format source data
SOURCE_DATA$`fig2D,S5C-D` <- u_dat %>%
  rename_with(.cols = all_of(ag_modules), ~ {
    .x %>%
      str_replace("_low", " Ag-low module") %>%
      str_replace("_high", " Ag-high module")
  }) %>%
          ...
          map_dfr(~ {
            dat %>%
              ...
              summarize(
                ...
                med_diff = abs(
                  median(value[ml_pred_1_grp == .x]) -
                    median(value[ml_pred_1_grp == p_con])
                ),
...
gene_examples <- lec_so %>%
  ...
  create_gn_plots(
    ...
    top_gns = c(
      ...
      "Col4a2", "Lama4",  # cell migration, angiogenesis
      "Vim", "Fn1"
...
B.  The top 5 gene ontology terms from each database
    (biological process, cellular component) are shown for the cLEC, fLEC, and
    collecting LEC Ag-high gene modules.
    The number of overlapping genes is shown to the right of each ontology term.
C.  Top gene ontology terms are shown for Ag-low gene modules as described in B.
D.  The expression of each Ag-high gene module is shown for naive LECs
    (0 days post immunization) along with timepoints from day 2, 14, 21, and 42.
    The center line, box limits, whiskers, and points represent the median,
    interquartile range (IQR), the range within 1.5×IQR from the box limits,
    and points outside this range (outliers), respectively.
E.  The expression of each Ag-low gene module is shown as described in D.
F.  The expression of the cLEC Ag-high gene module is shown for LEC and DC
    subsets for 14, 21, and 42 days post-immunization.
    Two-sided p values were calculated by comparing cLECs (marked by arrow)
    with each cell subset using a Wilcoxon rank sum test and
    adjusted using Benjamini-Hochberg correction. * p < 0.05, ** p < 0.001, *** p < 0.0001.
    Points indicate the median.
G.  The expression of the fLEC Ag-high gene module is shown as described in F.
H.  The expression of the collecting LEC Ag-high gene module is shown as
    described in F.
...
rf_venn <- MODULE_FEATS[[1]][rf_typs] %>%
  ...
  scale_fill_gradientn(colours = c("white", "#D7301F"), name = "number\nof genes") +
  theme(
...
go_res <- rf_grps %>%
  map_dfr(~ {
    ...
    go_feats <- go_feats %>%
      imap(~ {
        nms <- str_c(names(.), "-Ag-", grp, "-model ", .y)
        set_names(.x, nms)
      }) %>%
      flatten()
    ...
    go_sim_data %>%
      imap_dfr(~ {
        ...
        res@compareClusterResult %>%
          as_tibble() %>%
          mutate(
            cell_type   = str_extract(Cluster, "^[^-]+"),
            model_name  = str_extract(Cluster, "model [0-9]+"),
            gene_module = str_extract(Cluster, "Ag-[^-]+"),
            .after      = Cluster
          ) %>%
          mutate(db = .y, .after = Description)
      })
  })
...
go_fig <- go_sig %>%
  filter(model_name == "model 1") %>%
  group_by(db, Cluster, gene_module) %>%
  slice(1:5) %>%
  ungroup() %>%
  arrange(p.adjust) %>%
  split(.$gene_module) %>%
  imap(~ {
    ...
    dat %>%
      ggplot(aes(-log10(p.adjust), y_var, fill = cell_type)) +
      geom_col() +
      ...
      facet_grid(space = "free", scales = "free_y", rows = vars(cell_type)) +
      labs(title = .y, x = "-log10(p-value)") +
...
naive_dat <- naive_dat %>%
  ...
  mutate(
    module_type = str_c(module_type, " Ag-", module_ag, " module"),
...
naive_fig <- naive_dat %>%
  ...
  imap(~ {
    .x %>%
      ...
      geom_text(
        aes(3, Inf, label = module_type),
        data = ~ distinct(.x, subtype, module_type),
        vjust = 1.35
      ) +
      facet_wrap(~ subtype) +
      scale_fill_manual(values = lec_clrs) +
      scale_y_continuous(expand = expansion(c(0.05, 0.15))) +
      labs(x = "days post immunization", y = "module score") +
      base_theme +
      theme(
        aspect.ratio = 1.1,
        legend.position = "none"
...
# Format source data
list(high = "figS4D", low = "figS4E") %>%
  iwalk(~ {
    SOURCE_DATA[[.x]] <<- naive_dat %>%
      filter(module_ag == .y) %>%
      select(
        cell_id = .cell_id,
        tm,
...
# Create merged object containing subsets to compare
mod_so <- dc_so %>%
  subset(tm %in% rf_tms)
...
# Set module names
module_clmns <- set_names(
  str_c(names(MODULE_NAMES), seq_along(MODULE_NAMES)),
  names(MODULE_NAMES)
)

# Format n labels
dat <- mod_dat %>%
  pivot_longer(all_of(names(module_clmns)), names_to = "module") %>%
...
plts <- names(module_clmns) %>%
  grep("_high$", ., value = TRUE) %>%
  set_names() %>%
  map(~ {
    ttl   <- str_c(MODULE_NAMES[[.x]], " expression")
...
```{r "Fig S4", fig.width = 17, fig.height = 22}
figS4 <- plot_grid(
...
B.  Ag-low module score is shown for Ag-low, Ag-high, and predicted
    ...
    interquartile range (IQR), the range within 1.5×IQR from the box limits,
    and points outside this range (outliers), respectively.
C.  UMAP projections show fLEC Ag-high module scores for each timepoint (top)
    and Ag-high and Ag-low fLECs (bottom).
D.  UMAP projections show collecting LEC Ag-high module scores as described in
    C.
...
```{r "Fig S5", fig.width = 20, fig.height = 13}
top <- plot_grid(
  rf_bar_2, mod_bxs[[2]],
  rel_widths = c(0.75, 1),
  labels = c("A", "B"),
...
Training, validation, and testing metrics are included for classification
models.
...
tbl_s1 <- BEST_MLS %>%
  ...
  left_join(lb, by = c("subtype", "model_id", "algorithm")) %>%
  select(
    cell_type = subtype,
    model_name,
    model_id,
  ...
  mutate(
    ...
    cell_type  = fct_relevel(cell_type, rf_typs),
    data_split = fct_relevel(data_split, c("training", "validation", "testing")),
    model_name = str_c("model ", model_name)
  ) %>%
  arrange(cell_type, model_name, data_split)

# Write table
desc <- "Metrics are included for training, validation, and testing data. Two models were selected for each subset (GBM, DNN) based on validation metrics."

tbl <- list(
  description = tibble(description = desc),
  `table S1`  = tbl_s1
)

tbl %>%
  write_xlsx(
    here(params$table_dir, "table_s1.xlsx"),
...
tbl_s2 <- BEST_MLS %>%
  ...
  arrange(subtype) %>%
  mutate(
    across(c(model_name, subtype), ~ as.character(.x)),
    sheet = str_c(subtype, "-model ", model_name)
  ) %>%
  split(.$sheet) %>%
  map(~ {
    fts <- MODULE_FEATS[[.x$model_name]][[.x$subtype]]
    
    tbl <- .x$model[[1]]@model$variable_importances %>%
      as_tibble() %>%
      mutate(
        rank       = row_number(),
        subtype    = .x$subtype,
        model_name = str_c("model ", .x$model_name),
        algorithm  = .x$algorithm,
        .before    = 1
      ) %>%
      mutate(
        gene_module = map_chr(variable, ~ {
          case_when(
            .x %in% fts$high ~ "Ag-high",
            .x %in% fts$low  ~ "Ag-low",
            TRUE             ~ "none"
          )
        }),
...
# Write table
desc <- "Variables used to train models are listed along with relative importance. The top genes contributing to 99% of the cumulative importance were used for the Ag-low and Ag-high gene modules."

tbl <- append(
  list(description = tibble(description = desc)),
  tbl_s2
)

tbl %>%
  write_xlsx(
    here(params$table_dir, "table_s2.xlsx"),
    format_headers = FALSE
  )
...
Gene ontology terms are shown for Ag-low and Ag-high gene modules.
P values were calculated internally using a hypergeometric test and adjusted
using Benjamini-Hochberg correction.
Redundant GO terms were removed based on semantic similarity using the
...
# Write table
desc <- "Gene ontology terms are shown for Ag-low and Ag-high gene modules. P values were calculated internally using a hypergeometric test and adjusted using Benjamini-Hochberg correction. Redundant GO terms were removed based on semantic similarity using the clusterProfiler package."
...
tbl %>%
  write_xlsx(
    here(params$table_dir, "table_s3.xlsx"),
    format_headers = FALSE
  )
...
```{r "Fig 3", fig.height = 15, fig.width = 15}
img <- here("results/figures/other_panels/Figure 3 Clathrin and caveolin inhibitors.tif")
...
*Antigen archiving is enhanced by sequential immunizations*

A.  Experimental design
B.  Mean 21 day Ag-score is shown for LECs from mice that received a single
    immunization (21 day or 42 day) or dual immunization (21 day and 42 day).
C.  UMAP projections show 21 day Ag-scores for LEC subsets for single and dual
    immunizations.
D.  Prior immunization enhances antigen archiving.
    Ag-score is shown for single and dual immunizations for the 21 day timepoint
...
G.  Successive immunizations enhances retention of previously archived antigen.
    Ag-score is shown for the 42 day timepoint as described in C.
H.  21 day and 42 day Ag-score is compared for LEC subsets.
I.  The fraction of cells that archived antigens from one immunization (Ag-high)
    or both immunizations (double-high) is shown for single (day 14, 21, 42)
    and dual-immunized mice (dual).
I.  The expression of Ag-high modules described in Figure 2 is shown for LECs
    that archived antigens from both immunizations (double-high),
    from only one immunization (single-high),
...
```{r "exchange UMAPs", fig.width = 16, fig.height = 4, include = FALSE}
# Format Ag exchange plot data
# * for LECs use experiment 1 since experiment 2 is missing 3wk LECs
ex_dat <- ag_objs %>%
  imap(~ {
    dat <- .x@meta.data
    typ <- .y
    ...
    exps[exps != "exp-0"] %>%
      set_names() %>%
      map(~ {
        dat$Ag_class_dual <- dat$Ag_class_dual %||% as.character(NA)
        dat$ml_pred_1     <- dat$ml_pred_1     %||% as.character(NA)
        
        dat %>%
          as_tibble(rownames = ".cell_id") %>%
          filter(exp %in% c("exp-0", .x)) %>%
          mutate(
            mouse     = ex_key[as.character(mouse)],
            mouse     = fct_relevel(mouse, unname(ex_key)),
            replicate = rep_labs[as.character(exp)],
            subtype   = fct_relevel(subtype, ag_plt_typs[[typ]]),
            tm        = as.character(tm)
          ) %>%
          mutate(Ag_class_dual = recode(
            Ag_class_dual,
            low = "Ag-low",
            `single-high` = "Ag-high"
          )) %>%
          ...
          mutate(
            Ag_class_dual = if_else(
              vaccination == "dual" & subtype %in% rf_typs,
              Ag_class_dual,
              as.character(NA)
            )
          )
  ...
  pmap(~ {
    ...
    plt <- dat %>%
      plot_scatter(
        ...
        size         = 0.6,
        stroke       = 0.7,
        plot_colors  = c("lightblue", "white", args$clr),
        label_params = list(size = ttl_pt1)
      ) +
      guides(fill = guide_colorbar(
        ticks = FALSE,
...
# Format source data
# * immunization group is included only for group C and cell types used to train
#   models (cLEC, fLEC, collecting)
ex_source_dat <- ex_dat %>%
  map(~ {
    .x %>%
      map_dfr(~ {
        .x %>%
          filter(exp != "exp-0") %>%
          mutate(Ag_class_dual = replace_na(Ag_class_dual, "none")) %>%
          select(
            cell_id            = .cell_id,
            cell_type          = subtype,
            immunization_group = mouse,
            replicate,
            Ag_class           = Ag_class_dual,
            UMAP_1             = hUMAP_1,
            UMAP_2             = hUMAP_2,
            !!sym(ag_labs[[ag_clmns[[1]]]]) := !!sym(ag_clmns[[1]]),
            !!sym(ag_labs[[ag_clmns[[2]]]]) := !!sym(ag_clmns[[2]]),
            any_of(MODULE_NAMES_2)
          )
      })
  })
  
SOURCE_DATA$`fig4,S6A-B` <- ex_source_dat$LEC
...
heat_dat <- ex_dat$LEC[[ag_exps$LEC]] %>%
  filter(
    exp == ag_exps$LEC,
...
```{r "exchange boxes"}
# Effect of successive immunizations on antigen uptake/retention
# * use subsets shown in Fig 1
ex_bx_args <- list(
  tm      = c("21", "42"),
  tm_clmn = c("tm_3", "tm_6"),
...
# Create separate plots for each replicate and each Ag tag
ex_bxs <- ex_dat %>%
  imap(~ {
    typ <- .y
      ...
      imap(~ {
        ...
        ex_bx_args %>%
          pmap(~ {
            ...
            lbls <- c(
              single = ex_key[[tm_key_2[[args$tm]]]],
              dual   = ex_key[["6wk-3wk"]]
            )
            
            y_ttl <- str_c("Ag_", tm_key_2[[as.character(args$tm)]], "_score")
            y_ttl <- ag_labs[[y_ttl]]
            
            plt_typs <- ag_plt_typs[[typ]]
            
            if (identical(typ, "DC") && identical(exp, ag_exps$DC)) {
              plt_typs <- ag_plt_typs[[typ]][1:3]
            }
...
# Need to expand x axis so n labels fit on plot
ex_bxs <- ex_bxs %>%
  map(~ map(.x, ~ {
    .x[[2]] <- .x[[2]] +
...
# Plot correlation between Ag tags
ex_scat <- ex_dat$LEC %>%
  map(~ {
    .x %>%
      filter(
      ...
      create_exchange_scatter(
        x = c(Ag_3wk_score = ag_labs[["Ag_3wk_score"]]),
        y = c(Ag_6wk_score = ag_labs[["Ag_6wk_score"]])
      )
  })
```
...
ex_mod_bx_dat <- ex_dat$LEC %>%
  bind_rows() %>%
  filter(vaccination == "dual") %>%
  mutate(mouse = "6wk-3wk")

# Create boxplots
bx_lvls <- c("Ag-low", "Ag-competent", "Ag-high", "double-high")
...
x_labs <- ex_key %>%
  map_chr(str_remove, "^Group ")

x_labs[["d14"]] <- "d14-PI"

dat <- ex_mod_bx_dat %>%
  filter(subtype %in% rf_typs) %>%
  select(-ends_with("_correct")) %>%         # remove columns with inconsistent types
  mutate(ml_pred_1_grp = Ag_class_dual) %>%  # match column name with rf_bar_dat
  bind_rows(rf_bar_dat) %>%
  mutate(
    ml_pred_1_grp = fct_relevel(ml_pred_1_grp, rev(lvls)),
    subtype       = fct_relevel(subtype, rf_typs),
    mouse         = fct_relevel(mouse, names(m_key)),
    tm            = x_labs[as.character(mouse)]
  ) %>%
  arrange(mouse) %>%
  mutate(tm = fct_inorder(tm))
...
dual_ag_class_brs <- dat %>%
  ...
  scale_y_continuous(breaks = c(0, 0.5, 1)) +
  guides(fill = guide_legend(override.aes = list(shape = 22, size = 5))) +
  labs(y = "fraction of cells", x = "immunization group") +
  bar_thm +
  theme(
    aspect.ratio = 1.2,
    legend.title = element_blank(),
...
```{r "Fig 4 diagram", fig.width = 15.5, fig.height = 10}
img <- here("results/figures/experiment_design/Figure 4A.jpeg")
...
```{r "Fig 4", fig.width = 15.5, fig.height = 27}
fig4 <- fig4_dsgn /
  (ex_heat[[1]] + ex_u[[1]]) / ex_bxs$LEC[[ag_exps$LEC]][[1]] /
  (ex_heat[[2]] + ex_u[[2]]) / ex_bxs$LEC[[ag_exps$LEC]][[2]] /
  ex_scat[[ag_exps$LEC]] /
...
A.  Ag-score is shown for single and dual immunizations for the day 0 timepoint
    for LEC subsets from a biological replicate.
    Other timepoints are shown in grey.
    P values were calculated using a one-sided Wilcoxon rank sum test with
    Benjamini-Hochberg correction.
    This analysis was not performed for a biological replicate for the day 21
    timepoint due to the low number of LECs recovered (<40 total cells).
    The center line, box limits, whiskers, and points represent the median,
    interquartile range (IQR), the range within 1.5×IQR from the box limits,
    and points outside this range (outliers), respectively.
B.  Day 0 (BC1) and day 21 (BC2) Ag-score is compared for LEC subsets for mice
    that received dual vaccination.
    Values are shown for a biological replicate.
    The Spearman correlation between BC1 and BC2 is shown for each subset.
C.  LEGEND TEXT
D.  Spearman correlation between Ag-high module scores and day 0 and day 21 Ag
    scores is shown for LEC subsets.
    Two-sided p values were calculated and adjusted using Benjamini-Hochberg
    correction.
E.  The correlation between Ag-low module scores and Ag scores is shown as
    described in D.
...
# Format data
# * need to calculate p-values here so they can be included in the source
#   data table
scat_dat <- ex_mod_bx_dat %>%
  filter(subtype %in% rf_typs) %>%
  mutate(Ag_class_dual = recode(Ag_class_dual, `Ag-competent` = "Ag-low"))
...
# Format source data
# * include p values for spearman and pearson correlations to satisfy reviewer
#   comment
scat_p_dat <- names(MODULE_NAMES) %>%
  split(., str_detect(., "high")) %>%
  map(~ {
    scat_dat %>%
      .calc_cor_pvals(
        ag_clmns     = ag_clmns,
...
SOURCE_DATA$`figS6D-E` <- scat_dat %>%
  ...
  pivot_longer(
    all_of(ag_clmns),
    names_to = "Ag-tag", values_to = "Ag_score"
  ) %>%
  left_join(
    scat_p_dat,
    by = c(cell_type = "subtype", `Ag-tag` = "ag_clmn"),
    relationship = "many-to-many"
  ) %>%
  mutate(
    `Ag-tag`    = ag_labs_3[as.character(`Ag-tag`)],
    module_name = MODULE_NAMES[as.character(module_name)]
  ) %>%
  rename_with(
    .cols = starts_with("s_"),
    ~ str_replace(.x, "^s_", "spearman_")
  ) %>%
  rename_with(
    .cols = starts_with("p_"),
    ~ str_replace(.x, "^p_", "pearson_")
  ) %>%
  relocate(n, .after = Ag_class) %>%
  relocate(starts_with("spearman_"), .after = module_name)
```

```{r "Fig S6C"}
img <- here("results/figures/other_panels/supplemental S6C crop.tif")
...
```{r "Fig S6", fig.width = 13, fig.height = 17}
# Create final figure
# * only show day 42 tag signal since did recover enough LECs for day 21
bxs <- ex_bxs$LEC
bxs <- bxs[names(bxs) != ag_exps$LEC][[1]][[2]]

top <- wrap_plots(bxs, ncol = 1) /
  ex_scat$`exp-2` /
  figS6C /
...
B.  Localization of Ag+ and Ag- cells is shown for LECs for tissue sections
    from A.
C.  Localization of Ag+ and Ag- cells is shown for DCs for tissue sections from A.
D.  Cell-cell interaction enrichment results are shown for section 1 from A for
    Ag+/- LECs and DCs.
    Arrows indicate significant enrichment for LEC-DC interactions.
...
D.  Mean 21 day Ag-score is shown for DCs from mice that received a single
    immunization (21 day or 42 day) or dual immunization (21 day and 42 day).
E.  Ag-score is shown for single and dual immunizations for the 21 day timepoint
    for DC subsets with highest Ag-score.
    Other timepoints are shown in grey.
    P values were calculated using a one-sided Wilcoxon rank sum test with
    Benjamini-Hochberg correction.
F.  Mean 42 day Ag-score is shown for DCs from mice that received single or
    dual immunization as described in F.
G.  Ag-score is shown for single and dual immunizations for the 42 day timepoint
    as described in G.
...
xen_dat <- xen %>%
  filter(fov %in% c(top_fov, med_fov)) %>%
  mutate(
    type_ag = if_else(
    ...
    tm    = ex_key[as.character(mouse)],
    group = fov_grps[as.character(fov)],
    group = fct_relevel(group, unname(fov_grps))
  ) %>%
...
# Format source data
SOURCE_DATA$`fig5A-C,S7A,B,D,E` <- xen_dat %>%
  ...
  mutate(
    immunization_group = ex_key[as.character(immunization_group)]
  )
```
...
dat <- prox %>%
  ...
  mutate(
    ...
    x      = fct_inorder(x),
    significant = (enrichm < 0 & p.adj_lower < 0.05) |
      (enrichm > 0 & p.adj_higher < 0.05)
  )
...
prox_brs <- dat %>%
  ...
  map(~ {
    .x %>%
      ...
      geom_point(
        aes(y = 0),
        data = ~ filter(.x, lec_dc, significant),
        shape = 17,
        position = position_nudge(y = -0.2)
      ) +
...
prox_hists <- hist_dat %>%
  ...
  imap(~ {
    ...
    .x %>%  
      ...
      labs(x = "distance from nearest Ag+ LEC (\U00B5m+1)", y = "density") +
...
# Format source data
SOURCE_DATA$`fig5E,S7E` <- hist_dat %>%
  select(
    cell_id,
    slide,
    cell_type,
...
dc_heat_dat <- ex_dat$DC[[ag_exps$DC]] %>%
  filter(
    exp == ag_exps$DC,
...
# Format source data
SOURCE_DATA$`fig5F-I,S8E-F` <- ex_source_dat$DC
```

```{r "Fig 5", fig.width = 14, fig.height = 21}
bxs <- ex_bxs$DC[[ag_exps$DC]] %>%
...
A.  Localization of annotated cell types is shown for representative tissue
    sections showing the subcapsular sinus (shown in Figure 5) and the medullary
    sinus.
    Two adjacent tissue sections are shown for each (section 1 and 2).
    Annotated cell types with >200 identified cells are shown.
    Color indicates cell density.
B.  The localization of LEC subsets comprising the subcapsular (cLECs and fLECs)
    and medullary (Marco and Ptx3 LECs) sinuses is shown for tissue sections
    shown in A.
C.  Total Ag counts are shown for BC1 and BC2, for mouse immunization groups
    described in Figure 4.
    For each immunization group, tissue sections were analyzed for two LNs from
    separate mice (LN-1, LN-2), with the exception of the dual-immunization
    group.
    For each LN, two adjacent tissue sections were analyzed
    (section 1, section 2).
D.  The fraction of Ag+ cells is shown for each cell type for tissue sections
    shown in A.
E.  The fraction of Ag+ cells is shown for LEC subsets comprising the
    subcapsular (cLECs and fLECs) and medullary (Marco and Ptx3 LECs) sinuses
    for tissue sections shown in A.
    Replicate tissue sections were aggregated.
F.  Cell-cell interaction enrichment results are shown for Ag+/- LECs and DCs
    for a replicate (adjacent) tissue section for Figure 5D.
    Arrows indicate significant enrichment.
G.  The distance to the closest Ag+ LEC is shown for Ag+ and Ag- DCs for
    a replicate (adjacent) tissue section for Figure 5E.

```{r "xenium cell type sections", fig.width = 14, fig.height = 15}
# Format data for plots
xen_typs <- xen_dat$cell_type %>%
  table() %>%
  sort(decreasing = TRUE)

xen_typs <- xen_typs[xen_typs > 200]
xen_typs <- xen_typs[names(xen_typs) != "unassigned"]

# Function to set number of bins based on number of cells
fn <- approxfun(c(last(xen_typs), xen_typs[[1]]), c(65, 75))

one_line_strips <- function(x) str_replace(x, "\n", " ")
...
typ_plts <- names(xen_typs) %>%
  map(~ {
    plt <- xen_dat %>%
      .create_cell_heatmap(
        groups = .x,
        n_bins = fn(xen_typs[[.x]])
      ) +
      facet_grid(slide ~ group, labeller = as_labeller(one_line_strips))
    
    .add_umap_labels(plt, labels = c("x coord", "y coord"))
  })
...
# Format labels
xen_ag_labs <- ag_labs_3

names(xen_ag_labs) <- names(xen_ag_labs) %>%
  str_replace("score", "counts")

# Format colors
names(xen_ag_labs) %>%
  str_remove_all("^Ag_|_counts$")

xen_ag_clrs <- tm_key_2[as.character(names(tm_clrs))] %>%
  str_c("Ag_", ., "_counts")

xen_ag_clrs <- set_names(
  tm_clrs,
  xen_ag_labs[xen_ag_clrs]
)

# Format bargraph data
xen_bar_dat <- xen %>%
  filter(!fov %in% bad_fov) %>%
  pivot_longer(all_of(names(xen_ag_labs)), names_to = "Ag-tag") %>%
  ...
  ungroup() %>%
  mutate(
    `Ag-tag` = xen_ag_labs[as.character(`Ag-tag`)],
    `Ag-tag` = fct_relevel(`Ag-tag`, unname(ag_labs_3)),
    tm       = ex_key[as.character(mouse)]
  )
...
# Format source data
SOURCE_DATA$figS7C <- xen_bar_dat %>%
  select(
    slide,
    fov,
    immunization_group = tm,
    lymph_node = LN,
    `Ag-tag`,
    total_counts = value
  )
...
bar_dat <- xen_dat %>%
  ...
  group_by(cell_type, group, slide) %>%
  summarize(
...
xen_typ_bars <- bar_dat %>%
  mutate(
    ...
    group     = fct_reorder(group, pct_ag_pos, .fun = max, .desc = TRUE)
  ) %>%
  group_by(group, slide) %>%
...
```{r "xenium lec subsets sections", fig.width = 14, fig.height = 5}
# LEC subsets on sections
dat <- xen_dat %>%
  mutate(
    region = case_when(
      lec_type %in% c("cLEC", "fLEC")          ~ "cLEC/fLEC",
      lec_type %in% c("Marco_LEC", "Ptx3_LEC") ~ "Marco/Ptx3 LEC"
    )
  )

lec_clrs["other"] <- "grey80"

lvls <- c("cLEC", "fLEC", "Marco_LEC", "Ptx3_LEC", "tzLEC", "other")

lec_typ_sec <- dat %>%
  mutate(lec_type = fct_relevel(lec_type, rev(lvls))) %>%
  ggplot(aes(y_cell, x_cell, fill = lec_type)) +
  geom_point_trace(
    trace_position    = lec_type != "other",
    background_params = list(size = 0.5, color = NA),
    size   = 0.5,
    stroke = 0.7
  ) +
  facet_grid(slide ~ group, labeller = as_labeller(one_line_strips)) +
...
lec_typ_sec <- .add_umap_labels(lec_typ_sec, labels = c("x coord", "y coord"))
```

```{r "xenium Ag+ lec subsets"}
# Ag+ LEC subsets
clrs <- c(
  `cLEC/fLEC`      = "#D7301F",
  `Marco/Ptx3 LEC` = "#56B4E9"
)
...
reg_dat <- dat %>%
  ...
  mutate(
    frac = n / tot,
    region = fct_relevel(region, names(clrs))
  )

xen_lec_ag <- reg_dat %>%
  ggplot(aes(region, frac, fill = region)) + 
...
bot <- wrap_plots(
  lec_typ_sec  + labs(tag = "B"),
  xen_bars     + labs(tag = "C"),
  xen_typ_bars + labs(tag = "D"),
  design = "
  AAAABBB
  AAAABBB
  AAAACCC
...
prox <- wrap_plots(
  xen_lec_ag             + labs(tag = "E"),
  prox_brs$`section 2`   + labs(tag = "F") + theme(axis.title.x = element_text(vjust = 21)),
  prox_hists$`section 2` + labs(tag = "G"),
  nrow   = 1,
  widths = c(0.7, 1, 1)
) &
  theme(plot.tag = element_text(face = "bold", size = ttl_pt2 * 1.5))
...
figS7 <- plot_grid(
  typ_plts, bot, prox,
  ncol        = 1,
  rel_heights = c(0.95, 0.45, 0.25),
  labels      = c("A", "", ""),
  label_size  = ttl_pt2 * 1.5
)

figS7 %>%
  .save_fig("figS7.png", w = 14, h = 27)
...
The expression of LEC subset marker genes is shown below.

```{r "NEW MEDULLA BOXPLOTS", fig.width = 16, fig.height = 4}
mkrs <- c("Ackr4", "Madcam1", "Marco", "Ptx3")

bx_dat <- xen_dat %>%
  group_by(lec_type, group) %>%
  mutate(
    x_lab = n(),
    x_lab = ifelse(
      x_lab >= 10000,
      str_c(round(x_lab / 1000, 0), "k"),
      label_comma()(x_lab)
    ),
    x_lab = str_c(lec_type, " (", x_lab, ")")
  )

clrs <- lec_clrs
clrs["tzLEC"] <- lighten(clrs["tzLEC"], 0.3)

mkrs %>%
  map(~ {
    bx_dat %>%
      ggplot(aes(x_lab, !!sym(.x), fill = lec_type)) +
      geom_boxplot(outlier.size = 0.25) +
      facet_grid(slide ~ group, scales = "free_x") +
      scale_fill_manual(values = clrs) +
...
A.  Sample identity and annotated regions are shown for all regions analyzed
    using the GeoMx DSP platform.
B.  Representative tissue section is shown with annotated regions.
C.  Relative antigen tag signal is shown for each sample and each
    region segmented based on Lyve1 (LECs) and CD11C (DCs).
    Normalized 21 day antigen signal (BC2) was scaled across all regions from 21 day
    mice and dual immunized mice,
    0 day antigen signal (BC1) was scaled across all regions from 0 day mice and
    dual immunized mice.
...
E.  Ag-score is shown for single and dual vaccinations for the 21 day timepoint
    ...
    interquartile range (IQR), the range within 1.5×IQR from the box limits,
    and points outside this range (outliers), respectively.
F.  Ag-score is shown for single and dual vaccinations for the 0 day timepoint
    for a second biological replicate as described in E.

```{r "geomx knit setup", include = FALSE}
knit(here(params$template_dir, "setup-geomx.Rmd"), "")

# Format GeoMx data
# * segments from 6wk-3wk sample are plotted twice, signal for each of the two
#   tags is plotted separately
geomx_dat <- geo_ag_labs %>%
  imap_dfr(~ {
...
# Format source data
SOURCE_DATA$`figS8C-D` <- geomx_dat %>%
  select(
    Sample_ID,
    `Slide Name`,
    Segment,
  ...
  mutate(
    mouse    = ex_key[as.character(mouse)],
    `Ag-tag` = geo_ag_labs[as.character(`Ag-tag`)]
  )
```
...
```{r "Fig S8B"}
img <- here("results/figures/other_panels/For S8B.tif")
...
#   than Ag-6wk signal
# * segments from 6wk-3wk sample are plotted twice, signal for each of the two
#   tags is scaled and plotted
geo_bx_dat <- geomx_dat %>%
  group_by(key) %>%
  mutate(Ag_rel = as.numeric(scale(Ag_score)))
...
# Calculate positions for each p value label
p_dat <- p_dat %>%
  filter(p_adj < 0.05) %>%
  group_by(Segment) %>%
...
sct_labs <- sct_dat %>%
  ...
  mutate(n = str_c(geo_ag_labs[key], "\n", n))
...
```{r "Fig S8", fig.width = 14, fig.height = 18}
top <- plot_grid(
  geo_bxs, geo_sct,
  nrow   = 1,
  align  = "h",
...
bxs <- plot_grid(
  plotlist   = bxs[names(bxs) != ag_exps$DC][[1]],
  ncol       = 1,
  align      = "v",
  axis       = "rl",
  labels     = c("E", "F"),
...
figS8 <- plot_grid(
  ...
  top, bxs,
  ncol = 1,
  rel_heights = c(1, 0.7, 0.7, 1)
)

figS8 %>%
  .save_fig("figS8.png", w = 14, h = 18)
...
# Format source data
SOURCE_DATA$`fig6,S9` <- CHIKV_DAT %>%
  select(
    cell_id = .cell_id,
    treatment,
    rep,
    cell_type = !!sym(CHIKV_TYPE_CLMN),
    UMAP_1    = hUMAP_1,
    UMAP_2    = hUMAP_2,
    Ag_class_model_1 = ml_pred_1_grp,
    Ag_class_model_2 = ml_pred_2_grp,
    all_of(MODULE_NAMES_2),
    all_of(CHIKV_FEATS)
  )
```
...
top <- .add_umap_labels(top, arrows = TRUE)
```
...
heat <- heat_dat %>%
  ...
  base_theme +
  theme(
    aspect.ratio    = length(rf_typs) / length(CHIKV_FEATS) * 1.3,
...
```{r "Fig 6", fig.width = 12, fig.height = 15.5}
...
D.  The fraction of cLECs predicted to be Ag-competent is shown for each 
    MFLN and FL sample with at least 20 identified cLECs.
E.  The bargraphs show the median fraction of predicted Ag-compentent cells.
    Points show values for each sample.
    P values were calculated using a two-sided Wilcoxon rank sum test with
    Benjamini-Hochberg correction (middle).
F.  The fraction of predicted Ag-competent cells is shown for a second
    independent model as in E.

```{r "abe knit setup", include = FALSE}
knit(here(params$template_dir, "setup-abe.Rmd"), "")

# Format Abe data
# * include expression of key marker genes
abe_gns <- c(
  "ACKR4",
  "MFAP4",
  "SERPINE1",
...
abe_dat <- abe_lec_so %>%
  FetchData(c("sample", "tissue", "subtype", pred_clmns, abe_gns)) %>%
  as_tibble(rownames = "cell_id")

# Format source data
SOURCE_DATA$`fig7,S10` <- abe_dat %>%
  select(
    cell_id,
    sample,
    tissue,
    ...
    brs <- set_names(rf_typs) %>%
      imap(~ {
        ...
        dat %>%
          ...
          scale_y_continuous(breaks = c(0, 0.5, 1), limits = c(0, 1.001)) +
          labs(y = y_ttl) +
          base_theme +
          theme(
            plot.margin     = margin(5, 35, 5, 35),
            legend.position = "none",
            strip.clip      = "off",
            axis.title.x    = element_blank(),
            axis.text.x     = element_text(angle = 90, hjust = 1, vjust = 0.5)
    ...
    brs <- set_names(rf_typs) %>%
      map(~ {
        ...
        p <- dat %>%
          ...
          scale_y_continuous(
            breaks = c(0, 0.5, 1),
            limits = c(0, 1.1),  # if limit is 1, values of 1 are removed
...
```{r "Fig 7", fig.width = 13, fig.height = 3}
left  <- abe_bars_1[[1]][1]
right <- map(abe_bars_2, pluck, 1)

fig7 <- append(left, right) %>%
  map(~ .x + theme(plot.margin = margin(20, 20, 5, 5)))
...
fig7 %>%
  .save_fig("fig7.png", w = 13, h = 3.5)
```

```{r "Fig 7 complete", fig.width = 13, fig.height = 15}
img <- here("results/figures/other_panels/Figure 7 New CHIKV figure with human LN data.tif")
...
B.  Ag-high and Ag-low module scores are shown for mock and CHIKV-infected mice
    ...
    P values were calculated using a two-sided
    Wilcoxon rank sum test with Benjamini-Hochberg correction.
    Boxplots were drawn as described in A.
...
```{r "Fig S9", fig.width = 13.5, fig.height = 15}
...
figS9 %>%
  .save_fig("figS9.png", w = 13.5, h = 15, mtplyr = 0.1)
...
B.  The fraction of fLECs predicted to be Ag-competent is shown for each 
...
```{r "abe marker expression", include = FALSE}
# Format marker data
abe_gn_dat <- abe_dat %>%
  filter(subtype != "unassigned") %>%
...
```{r "Fig S10", fig.width = 13, fig.height = 9}
left <- abe_bars_1[[1]][-1] %>%
  plot_grid(
    plotlist   = .,
    ncol       = 1,
    ...
    axis       = "rl",
    labels     = c("B", "C"),
    label_size = ttl_pt2 * 1.5, label_x = -0.01
  )
...
abe_bars <- plot_grid(
  plotlist   = append(list(left), right),
  nrow       = 1,
  rel_widths = c(1, 0.3, 0.3),
  align      = "h",
  axis       = "tb"
...
top <- plot_grid(
  NULL, abe_gn_heat, NULL,
  rel_widths = c(0.35, 1, 0.35),
  nrow       = 1,
  labels     = c("", "A", ""),
  label_size = ttl_pt2 * 1.5
)
...
SOURCE_DATA %>%
  write_xlsx(
    here(params$table_dir, "source_data.xlsx"),
    format_headers = FALSE
  )
```
