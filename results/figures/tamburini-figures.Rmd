...
A.  Antigen counts are shown for replicate LN tissue sections analyzed with the
    Xenium platform.
B.  Localization of Ag+ and Ag- cells is shown for LECs and DCs.
C.  The distance to the closest Ag+ LEC is shown for Ag+ and Ag- DCs.
...
B.  The fraction of Ag+ cells is shown for each cell type.
...
* Samples with >20 cLECs are included in the analysis
* Most FL samples have <100 cLECs

```{r "abe lec subsets", eval = FALSE}
abe_gns <- c(
  "ACKR4",
  "CXCL5",
  "BMP2",
...
abe_lec_so %>%
  FetchData(c("type", abe_gns)) %>%
  as_tibble(rownames = ".cell_id") %>%
  filter(grepl("^LEC[0-9]+$", type)) %>%
  ...
  mutate(value = as.numeric(scale(value))) %>%
  ungroup() %>%
  mutate(name = fct_relevel(name, rev(abe_gns))) %>%
  
  ggplot(aes(type, name, fill = value)) +
  geom_tile() +
  base_theme

```

```{r "Fig S9", fig.width = 10, fig.height = 4.5, out.width = "75%"}
...
abe_bars_1 <- abe_bar_dat %>%
  ...
  labs(y = str_c("fraction Ag-competent ", abe_type)) +
  base_theme +
  theme(
    legend.position = "none",
    axis.title.x = element_blank(),
...
heat_dat %>%
  ...
  theme(
    legend.title = element_blank(),
    aspect.ratio = length(cd45pos_gns) / n_distinct(dat$cell_type),
    axis.title   = element_blank(),
    axis.text.x  = element_text(angle = 90, hjust = 1, vjust = 0.5)
  )
```
...
heat_dat %>%
  ...
  theme(
    legend.title = element_blank(),
    aspect.ratio = length(cd45neg_gns) / n_distinct(dat$cell_type),
    axis.title   = element_blank(),
    axis.text.x  = element_text(angle = 90, hjust = 1, vjust = 0.5)
  )
```
...
#   mutate_meta(
#     mutate,
#     new_type = if_else(pred_conf > 0.5, rough_type, "unassigned")
#   ) %>%
#   classify_markers(
#     feats      = "Car3",
#     filt       = Car3 > 0.75,
...
#     mutate,
#     new_type = if_else(
#       new_type %in% c("unassigned", "Epithelial cells"),
#       str_c("other ", cd45_class),
#       new_type
#     ),
#     Ag_type = ifelse(Ag_class == "Ag-high", new_type, "Ag-low")
#   ) %>%
# 
#   mutate(
#     new_type = if_else(
...
# 
# xen <- xen %>%
#   mutate(cell_type = new_type)
#   
# xen %>%
#   # filter(fov == top_fov) %>%
#   filter(fov == top_fov, Ag_class == "Ag+") %>%
#   pull(new_type) %>%
#   table()
