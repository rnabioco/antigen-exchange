...
tbl_s2 <- BEST_MLS %>%
  ...
  arrange(subtype) %>%
  mutate(
    across(c(model_label, subtype), ~ as.character(.x)),
    sheet = str_c(subtype, "-model ", model_label)
  ) %>%
  split(.$sheet) %>%
  map(~ {
    fts <- MODULE_FEATS[[.x$model_label]][[.x$subtype]]
    ...
    tbl <- .x$model[[1]]@model$variable_importances %>%
      ...
      mutate(
        rank       = row_number(),
        subtype    = .x$subtype,
        model_name = str_c("model ", .x$model_label),
