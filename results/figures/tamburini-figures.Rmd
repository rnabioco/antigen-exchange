...
params:
  ...
  mod_dir:         "~/Dropbox/Ryan/Projects/antigen-exchange/results/models/2025-02-03_2" # ML models
  ...
  table_dir:       "results/tables/2025-02-03_2"                                          # tables
...
- [ ] &nbsp;Train additional models to further corroborate predictions and check
      contribution of Cav/Cavin genes ([R3C7](#R3C7))
- [ ] &nbsp;Perform additional Xenium analysis to further assess spatial
      correlation between Ag+ DCs and LECs ([R3C8](#R3C8))
- [x] &nbsp;Revise cell type annotations, check epithelial cells ([R3C3](#R3C3))
- [x] &nbsp;Compare expression of the Ag modules for LECs vs other stromal cells
      and DCs ([R3C5](#R3C5))
...
knit(here(params$template_dir, "setup-automl-2.Rmd"), "")
...
tbl_s2 <- best_mls %>%
  ...
  pmap_dfr(~ {
    ...
    tibble(
      ...
      mutate(
        Ag_class = case_when(
          gene %in% MODULE_FEATS[[1]][[typ]]$high ~ "Ag-high",
          gene %in% MODULE_FEATS[[1]][[typ]]$low  ~ "Ag-low"
        )
      )
  })
...
figS6 <- plot_grid(
  ...
  all_chikv_bars,  other_chikv_bars[[1]],
  labels = c("A", "B", "C", "D"),
  label_size = ttl_pt2 * 1.5,
  nrow  = 2,
  align = "vh",
    ...
    brs <- set_names(rf_typs) %>%
      map(~ {
        ...
        abe_bar_dat <- abe_lec_so@meta.data %>%
          ...
          summarize(
            frac_high = sum(!!sym(pred_clmn) == "Ag-competent"),
            .groups = "drop"
          ) %>%
          mutate(
