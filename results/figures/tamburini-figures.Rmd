...
A.  The top 15 gene ontology terms are shown for Ag-high gene modules for each
    LEC subset.
B.  Top gene ontology terms are shown for Ag-low gene modules as described in E.
C.  The fraction of cells predicted to be Ag-low and Ag-high is shown for each
    LEC subset.
D.  Ag-low module score is shown for Ag-low, Ag-high, and predicted
    Ag-competent LECs.
    The Spearman correlation between Ag classes and Ag-high module score is
    shown for each timepoint.
    One-sided p values were calculated and adjusted using Benjamini-Hochberg
    correction.
E.  UMAP projections show fLEC Ag-high module scores for each timepoint.
F.  UMAP projections show collecting LEC Ag-high module scores for each
    timepoint.
