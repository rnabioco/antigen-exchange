...
- [x] &nbsp;Adjust [Figure 1] and [Figure S1] colors to make them more distinguishable
      ([R2-minor](#R2-minor)).
- [ ] &nbsp;Generate source data tables for all figures.
...
# Format source data
SOURCE_DATA$fig1B <- heat_dat %>%
  select(tm, cell_type, `Ag_score mean`) %>%
  arrange(desc(tm), cell_type)
```
  ...
  map(~ {
    .x %>%
      map_dfr(~ {
        .x %>%
          filter(exp != "exp-0") %>%
          rename_with(.cols = matches("_(low|high)$"), ~ MODULE_NAMES[.x]) %>%
          select(
            cell_id            = .cell_id,
            cell_type          = subtype,
...
bxs <- plot_grid(
  plotlist = rep_ex_bxs[names(rep_ex_bxs) == "DC"],
  ncol   = 1,
  align  = "v",
  axis   = "rl",
  labels = c("E", "F"),
