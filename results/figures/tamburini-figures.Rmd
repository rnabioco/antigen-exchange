  ...
  map(~ {
    ...
    exps[exps != "exp-0"] %>%
      map_dfr(~ {
        dat %>%
          filter(exp %in% c("exp-0", .x)) %>%
          mutate(exp = rep_labs[as.character(exp)])
      })
  })
...
# Format source data
SOURCE_DATA$`fig5A-C,S7A,C` <- xen_dat %>%
  select(
    cell_id,
    fov,
    slide,
...
# Format source data
SOURCE_DATA$figS7B <- xen_bar_dat %>%
  select(
    slide,
    fov,
...
```{r "Fig S7", fig.width = 14, fig.height = 18}
bot <- wrap_plots(
  xen_bars + labs(tag = "B"),
...
Cell-cell interaction enrichment results are shown for Ag+ and Ag- cells
identified using the Xenium spatial transcriptomics platform.
