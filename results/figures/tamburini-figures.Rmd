...
naive_fig <- naive_dat %>%
  ...
  imap(~ {
    .x %>%
      ...
      facet_wrap(subtype ~ module_type) +
      scale_fill_manual(values = lec_clrs) +
...
img <- "results/figures/experiment_design/Figure 4A.jpeg"
img <- readJPEG(here(img))
img <- rasterGrob(img, interpolate = TRUE)
...
# Create an empty plot for placing the image, adjusted to fill the area
fig4_dsgn <- ggplot(img_lims, aes(x, y)) + 
  annotation_custom(
    grob = img,
    xmin = -Inf, xmax = Inf,
    ymin = -Inf, ymax = Inf
...
```{r "Fig S6", fig.width = 13, fig.height = 16}
# Create final figure
...
figS9 %>%
  .save_fig("figS9", w = 13.5, h = 15, mtplyr = 0.1)
