...
figS3 %>%
  .save_fig("figS3.tif", w = 15, h = 17.5)
...
C.  The expression of each Ag-high gene module is shown for naive LECs
    (0 days post immunization) along with each time point.
...
A.  Ag-score is shown for single and dual immunizations for the 21 day timepoint
...
B.  The localization of LEC subsets comprising the subcapsular (cLECs and fLECs)
    ...
    shown in A.
C.  Total Ag counts are shown for BC1 and BC2, for mice immunization groups
...
# xen_mkrs <- c("Ackr4", "Marco") %>%
#   map(~ {
#     dat %>%
#       arrange(!!sym(.x)) %>%
#       ggplot(aes(y_cell, x_cell, fill = !!sym(.x))) +
#       geom_point_trace(
#         trace_position    = lec_type != "other",
#         background_params = list(size = 0.001, color = NA),
#         size = 0.001, stroke = 0.7
#       ) +
#       facet_grid(slide ~ group) +
#       scale_fill_gradientn(
#         colours = c("lightblue", typ_clrs[["LEC"]]),
#         name    = str_c(.x, " expression")
#       ) +
#       coord_equal() +
#       umap_theme_2 +
#       theme(
...
# 
# xen_mkrs <- wrap_plots(
#   append(list(lec_typ_sec), xen_mkrs),
#   nrow = 1
# )
# 
# # LEC subset frequency
# xen_lec_brs <- dat %>%
#   filter(cell_type == "LEC") %>%
#   ggplot(aes(slide, fill = lec_type)) + 
...
figS9 %>%
  .save_fig("figS9.tif", w = 13.5, h = 15, mtplyr = 0.1)
