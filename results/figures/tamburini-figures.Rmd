...
params:
  ...
  xen_res_dir: "results/xenium/20240322__194506__032224_Tamburini_Run_1"
  xen_regex:   "output-XETG00230__0022624__[A-Z]__"
  # Sample identity with 6wk and 3wk barcode identity (listed in this order)
  # * `1` indicates GeoMx-Barcode01
  xen_samples: 
    value:
      A: ["6wk", 1, 2]
      B: ["6wk", 2, 1]
...
```{r "XENIUM DATA", eval = FALSE}
# Load objects
xen   <- qread(here(params$xen_dir, "xen_s1.qs"))
small <- qread(here(params$xen_dir, "small_s1.qs"))
...
xen_clrs <- xen_clrs[names(xen_clrs) %in% xen_dat$cell_type]
# xen_clrs <- c(xen_clrs, dc_clrs)
```

```{r "XENIUM BAD", eval = FALSE}
library(ggspruce)

typ_clrs["unassigned"] <- "grey65"
...
xen_clrs <- typ_clrs[names(typ_clrs) %in% xen_dat$cell_type]
xen_clrs <- c(xen_clrs, dc_clrs)

xen_clrs <- xen_clrs %>%
  spruce_up_colors(
    adjust_colors = "Siglec-H DC",
    difference    = 30
  )

# Plot all cells for tissue section
full_fig <- xen_dat %>%
  .plot_full_fov(
    fov   = "D",
    color = "cell_type",
    trace = list(c("LEC", dcs)),
    clrs  = xen_clrs
  )
```

```{r "XENIUM AG CLASS", eval = FALSE}
# Format plot data
dat <- xen@meta.data %>%
  filter(fov == top_fov) %>%
  mutate(
    ...
    Ag_class  = fct_relevel(Ag_class, c("Ag-high", "Ag-low")),
    type_ag   = if_else(
      broad_type %in% c("LEC", "DC"),
      str_c(broad_type, " (", Ag_class, ")"),
      broad_type
    ),
    Ag_counts = Ag_tag_1 + Ag_tag_2
  ) %>%
...
names(new_clrs) <- c(
  "LEC (Ag-low)", "LEC (Ag-high)",
  "DC (Ag-low)", "DC (Ag-high)",
  "other"
)
...
```{r "XENIUM PROXIMITY", fig.width = 18, fig.height = 13, eval = FALSE}
# Ag class labels
ag_classes <- c(
  `Ag-low`  = "Ag-",
  `Ag-high` = "Ag+"
...
```{r "XENIUM AG TYPE HEAT", eval = FALSE, fig.width = 14, fig.height = 6, eval = FALSE}
bar_dat <- xen@meta.data %>%
  mutate(Ag_counts = Ag_tag_1 + Ag_tag_2) %>%
  group_by(fov, cell_type) %>%
  summarize(
    n = n(),
    pct_ag_pos = (sum(Ag_counts > 0) / n) * 100,
    .groups = "drop"
  ) %>%
  filter(n > 100)
...
```{r "XENIUM AG TYPE BARS", eval = FALSE, fig.width = 14, fig.height = 6, eval = FALSE}
bar_dat <- xen@meta.data %>%
  mutate(Ag_counts = Ag_tag_1 + Ag_tag_2) %>%
  group_by(fov, cell_type) %>%
  summarize(
...
br <- bar_dat %>%
  ...
  mutate(cell_type = fct_reorder(cell_type, pct_ag_pos)) %>%
  ggplot(aes(cell_type, pct_ag_pos, fill = cell_type)) +
  geom_col() +
