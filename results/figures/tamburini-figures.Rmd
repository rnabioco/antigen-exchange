...
```{r "SPECIFICITY", eval = FALSE}
progs <- rf_typs %>%
  map(~ str_c(.x, "_", c("high", "low"))) %>%
  c(recursive = TRUE)

dat <- lec_so@meta.data %>%
  filter(subtype %in% rf_typs) %>%
  mutate(pred_grp = fct_relevel(pred_grp, names(pred_clrs))) %>%
  filter(
    tm == "14",
    pred_grp == "Ag-high"
  ) %>%
  pivot_longer(all_of(progs))
...
# p-values
x <- dat %>%
  filter(name == "cLEC_low")

wilcox.test(
  (filter(x, subtype == "cLEC"))$value,
  (filter(x, subtype == "Collecting"))$value
)
...
```{r "ABE ALT BARGRAPHS", eval = FALSE}
# Create Ag class bargraphs
abe_type <- "cLEC"
...
abe_lec_so@meta.data %>%
  ...
  filter(subtype == abe_type) %>%
  group_by(tissue) %>%
  mutate(rep = str_extract(orig.ident, "[0-9](?=_)")) %>%
  create_chikv_class_bars(
    x    = "tissue",
    n_p  = n_distinct(abe_lec_so$subtype),
    clrs = abe_clrs,
    ttl  = str_c("fraction Ag-competent ", abe_type, "s")
  ) +
  theme(strip.text = element_blank())
```

```{r "ABE MODULE SCORES", eval = FALSE}
names(module_nms) %>%
  map(~ {
    abe_lec_so@meta.data %>%
      filter(subtype %in% rf_typs) %>%
      ggplot(aes(sample, !!sym(.x), fill = tissue)) +
      geom_boxplot() +
...
abe_lec_so@meta.data %>%
  ...
  mutate(pred_grp = fct_relevel(pred_grp, names(pred_clrs))) %>%
  arrange(pred_grp) %>%
  ggplot(aes(Collecting_low, Collecting_high, color = pred_grp)) +
  # ggplot(aes(cLEC_low, cLEC_high, color = pred_grp)) +
...
abe_lec_so@meta.data %>%
  filter(subtype %in% rf_typs) %>%
  ggplot(aes(sample, Collecting_low, fill = pred_grp)) +
  geom_boxplot() +
  facet_wrap(~ subtype) +
  base_theme +
  theme(
