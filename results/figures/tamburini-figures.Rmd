...
params:
  ...
  mod_dir:      "~/Dropbox/Ryan/Projects/antigen-exchange/results/models/2024-07-01" # RF models
  ...
  table_dir:    "results/tables/2024-07-01"                                          # tables
...
```{r "Fig S8", fig.width = 12, fig.height = 4.5}
# Plot fraction Ag-competent cLECs
abe_bar_dat <- abe_lec_so@meta.data %>%
  rownames_to_column(".cell_id") %>%
  filter(type == "LEC1") %>%
  mutate(tissue = fct_relevel(tissue, names(abe_clrs)))
...
abe_bars_2 <- abe_bar_dat %>%
  ...
  theme(
    aspect.ratio = 1.5,
    strip.text   = element_blank(),
    axis.text.x  = element_text(hjust = c(0.65, 0.35))
  )
