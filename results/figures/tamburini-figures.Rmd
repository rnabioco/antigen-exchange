...
xen_clrs["unassigned"]  <- "white"
xen_clrs["other CD45+"] <- "white"
xen_clrs["other CD45-"] <- "white"
...
Mean expression is shown for key marker genes for CD45+ datasets.
...
dat <- cd45_so %>%
  FetchData(c("cell_type", "mouse", gns)) %>%
  as_tibble(rownames = ".cell_id") %>%
...
heat_dat %>%
  ...
  theme(
    ...
    aspect.ratio = length(gns) / n_distinct(dat$cell_type),
...
n_labs <- dat %>%
  ...
  pull(n_lab, cell_type)

# Plot cell types
xen_typ_fig <- dat %>%
  ggplot(aes(y_cell, x_cell, fill = cell_type)) +
  geom_point_trace(
    size = 0.1,
    color = NA,
    trace_position = !grepl("^other", cell_type),
    background_params = list(color = "black", size = 0.5)
  ) +
  facet_wrap(~ slide) +
  ...
  scale_fill_manual(values = xen_clrs, labels = n_labs) +
  coord_fixed() +
  umap_theme_2 +
  theme(
    legend.position   = "bottom",
...
dat <- xen %>%
  ...
  mutate(
    cell_type = if_else(
      cell_type %in% c("LEC", "DC"),
      cell_type,
      "other"
    ),
    cell_type = fct_relevel(cell_type, c("LEC", "DC", "other")),
    
    Ag_class = fct_relevel(Ag_class, rev(unname(xen_ag_classes))),
    
    type_ag = if_else(
      cell_type %in% c("LEC", "DC"),
      str_c(cell_type, " (", Ag_class, ")"),
      cell_type
    )
  ) %>%
  arrange(cell_type, Ag_class) %>%
...
hist <- hist_dat %>%
  ...
  facet_wrap(~ slide, ncol = 1) +
  labs(x = "distance from nearest Ag+ LEC (\U00B5m)", y = "density") +
  scale_fill_manual(values  = c(`Ag-` = "grey85", `Ag+` = "#D7301F")) +
  scale_color_manual(values = c(`Ag-` = "black",  `Ag+` = "#D7301F")) +
...
```{r "Fig 4", fig.width = 14, fig.height = 16}
bot <- list(
  E   = dc_ex_heat[[1]],
  `F` = dc_ex_bxs[[1]],
  G   = dc_ex_heat[[2]],
...
```{r "xenium cell type umaps", eval = FALSE}
typs <- xen$cell_type %>%
  table() %>%
...
fn <- approxfun(c(last(typs), typs[[1]]), c(50, 150))
...
x <- names(typs) %>%
  map(~ {
    ...
    dat %>%
      mutate(slide = str_c("section ", slide)) %>%
      ggplot(aes(x_cell, y_cell)) +
      geom_point_trace(fill = "white", trace_position = "bottom") +
      
      geom_bin2d(
        data = ~ filter(.x, cell_type == typ),
        # bins = 150
        bins = floor(fn(typs[[typ]]))
      ) +
      ...
      # geom_pointdensity(data = ~ filter(.x, cell_type == typ)) +
      # geom_density_2d(geom = "raster", data = ~ filter(.x, cell_type == typ)) +
      ggtitle(str_replace_all(typ, "_", " ")) +
      scale_fill_gradientn(colours = c("white", typ_clrs[[typ]])) +
...
```{r "fraction 3wk/6wk Ag+ cells", fig.height = 3.5, fig.width = 6, out.height = "50%"}
bar_dat <- xen %>%
...
```{r "XENIUM AG TYPE HEAT ALL", fig.width = 14, fig.height = 6}
bar_dat <- xen %>%
...
The percent of cells that are Ag+ is shown for each cell type for tissue section
D.
Only cell types with >100 cells identified for the tissue section are shown.

```{r "XENIUM AG TYPE HEAT D", fig.width = 14, fig.height = 6}
bar_dat <- xen %>%
  filter(fov == "D") %>%
  group_by(cell_type, fov, slide) %>%
  filter(n() > 100) %>%
...
```{r "geomx all Ag boxplots", fig.width = 10, fig.height = 6.5, eval = FALSE}
# Create boxplot
...
```{r "geomx cropped images", fig.width = 14, fig.height = 7, eval = FALSE}
...
```{r "geomx figure", fig.width = 14, fig.height = 10, eval = FALSE}
bot <- plot_grid(
  geo_bxs, geo_sct,
  nrow   = 1,
  align  = "h",
