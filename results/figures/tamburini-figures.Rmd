...
knitr::knit(here::here(params$template_dir, "setup-abe.Rmd"), "")
...
```{r "XENIUM PROXIMITY", fig.width = 18, fig.height = 13}
# Ag class labels
ag_classes <- c(
  `Ag-low`  = "Ag-",
  `Ag-high` = "Ag+"
...
```{r "XENIUM AG TYPE HEAT", eval = FALSE, fig.width = 14, fig.height = 6}
...
```{r "XENIUM AG TYPE BARS", eval = FALSE, fig.width = 14, fig.height = 6}
bar_dat <- xen@meta.data %>%
  mutate(Ag_counts = Ag_tag_1 + Ag_tag_2) %>%
  group_by(fov, cell_type) %>%
  summarize(
