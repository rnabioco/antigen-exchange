...
```{r "xenium cell types", eval = FALSE}
# Format data
dat <- xen %>%
  filter(fov == top_fov)
...
```{r "xenium Ag counts"}
plot_xenium <- function(df_in, dat_clmn, grp_clmn = NULL, clrs,
  ...
  is_num <- is.numeric(df_in[[dat_clmn]])
...
new_clrs <- c("white", "#56B4E9", "#E7301F") %>%
  expand_colors(
    n             = c(1, 2, 2),
    keep_original = TRUE,
...
```{r "xenium cell type umaps", fig.width = 14, fig.height = 10, eval = FALSE}
# Format data for plots
dat <- xen %>%
  filter(fov == top_fov) %>%
  
  filter(pred_conf <= 0.5 & pred_conf > 0.4) %>%
  mutate(cell_type = rough_type)
...
typs <- typs[typs > 100]
typs <- typs[!grepl("^other ", names(typs))]

# Function to set number of bins based on number of cells
fn <- approxfun(c(last(typs), typs[[1]]), c(45, 100))
...
bar_dat <- xen@meta.data %>%
  ...
  mutate(
    cell_type = new_type,
    cell_type = ifelse(grepl("^other", cell_type), "unassigned", cell_type),
    slide = "1"
  ) %>%
...
```{r "Fig S6", fig.width = 14, fig.height = 12}
bot <- plot_grid(
  geo_bxs, geo_sct,
  nrow   = 1,
  align  = "h",
...
```{r "XENIUM MARKER TEST"}
# Adjust B and T cell annotations
xen <- xen %>%
  mutate_meta(mutate, new_type = cell_type) %>%
  classify_markers(
    feats      = c("Cd19", "Cd3d"),
    filt       = Cd19 > 1.1,
    type_label = "B cells",
    clst_col   = str_c("SCT_snn_res.", xen_clst_rsln),
    type_col   = "new_type"
  ) %>%
  classify_markers(
    feats      = c("Cd19", "Cd3d"),
...
xen_gns %>%
  ...
  # mutate(
  #   new_type = if_else(Cd3d > 0.5, "T cells", new_type),
  #   new_type = if_else(Cd19 > 1.5, "B cells", new_type)
  # ) %>%
  
  filter(
    (new_type == "DC" & Sirpa > 0) | new_type != "DC"
  ) %>%
  
  pivot_longer(any_of(gns)) %>%
  ggplot(aes(new_type, value, fill = new_type)) +
  geom_boxplot() +
  facet_wrap(~ name) +
  base_theme +
