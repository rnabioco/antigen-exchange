...
naive_fig <- fig_dat %>%
  ...
  imap(~ {
    ...
    plt <- pt_dat %>%
      ggplot(aes(tm, value, color = subtype)) +
      geom_boxplot(
        color         = ln_col,
        fill          = ln_col,
        width         = 0.25,
        linewidth     = 0.75,
        outlier.color = ln_col,
        outlier.size  = 0.001
      ) +
      stat_summary(geom = "point", fun = median, size = 3.2)
      # geom_point(
...
ex_mod_bxs <- rf_mod_args %>%
  pmap(~ {
    ...
    ex_mod_bx_dat %>%
      ...
      theme(plot.margin = margin(0, 15, 5, 15))
  }) %>%
  wrap_plots(nrow = 1)
```
...
```{r "FIG 5 CSHL POSTER", fig.width = 12, fig.height = 8.5}
bot <- plot_grid(
  geo_bxs, geo_sct,
  nrow   = 1,
  align  = "h",
