...
.plot_full_fov_signal <- function(df_in, fov_in, color,
                                  clrs = c("white", "#D7301F"),
  ...
  res <- dat %>%
    arrange(!!sym(color)) %>%
    ggplot(aes(y_cell, x_cell, fill = !!sym(color) + pseudo)) +
    geom_point_trace(
      size = 1,
      stroke = 0.5,
      ...
...
plt_cells <- xen@meta.data %>%
  filter(cell_type %in% top_types | Ag_km == "Ag-high" | chikv_class == "CHIKV-high") %>%
  rownames() %>%
  c(plt_cells) %>%
  unique()
...
* LECs show strongest enrichment of Ag-high and CHIKV+ cells
* Samples infected with WT CHIKV show highest levels of viral RNA
* CHIKV WT sample shows higher relative levels of viral sgRNA
* Ag signals are highest in CHIKV 181/25 samples when compared to WT CHIKV

<u>To do</u>

* Refine cell type annotations
* Annotate DC subsets, perform Ag-exchange analysis
* Assess background signals
...
qc_dat <- xen %>%
  FetchData(c(feats, params$custom_targets), slot = "counts") %>%
  as_tibble(rownames = ".cell_id") %>%
  pivot_longer(all_of(params$custom_targets))
...
```{r "SLIDE 1: cell type sections", fig.width = 15.5, fig.height = 16}
# Legend labels
...
High resolution images show all cells (left) or all cells with LECs highlighted
(right).
...
High-resolution images showing showing all cells are included at the bottom

```{r "SLIDE 5: LEC type sections", fig.width = 15, fig.height = 12}
# Legend labels
...
CHIKV+ cells (>0 CHIKV counts) are shown for LN sections.
...
The fraction of CHIKV+ cells is shown for each sample.
...
type_dat %>%
  ...
  mutate(
    cell_type = fct_reorder(cell_type, frac, max, .desc = TRUE)
  ) %>%
  group_by(cell_type) %>%
  filter(
    "CHIKV WT" %in% sample,
  ...
  ggplot(aes(sample, frac, fill = sample)) +
  geom_col() +
...
```{r "CHIKV SIGNAL SAMPLES", fig.width = 15, fig.height = 4, out.height = "50%"}
args <- list(
...
```{r "CHIKV SIGNAL SAMPLES 2", fig.width = 15, fig.height = 4, out.height = "50%"}
plot_grid(
  plotlist = plts[3:4],
  nrow  = 1,
  align = "h",
...
```{r, fig.width = 20, fig.height = 10, results = "asis", out.height = "25%"}
...
Ag counts are shown below for LECs with >=1 Ag count.
LECs were divided into Ag-low and Ag-high groups.
The number of Ag-high LECs and total LECs is shown for each sample.
Replicates were pooled together.
...
Ag-high cells are shown for each LN tissue section.
The fraction of cells that are Ag-high is shown on the right.
...
High-resolution images show Ag counts for each tissue section.

```{r, fig.width = 20, fig.height = 10, results = "asis", out.height = "50%"}
plt_fovs <- xen_dat %>%
  group_by(fov) %>%
  filter(sum(Ag_tag) > 100) %>%
  pull(fov) %>%
  unique() %>%
  as.character()
...
### Ag signal is reduced in CHIKV WT samples
...
```{r "CHIKV+ Ag-high", eval = FALSE}
xen_dat %>%
  filter(grepl("Ag$", sample)) %>%
  group_by(sample, cell_type, chikv_class) %>%
  summarize(
    n = n(),
    n_ag = sum(Ag_km == "Ag-high"),
    .groups = "drop"
...
```{r "XENIUM EXAMPLES", eval = FALSE}

xen_dat %>%
  filter(fov == "Group_B_6") %>%
  arrange(frac_CHIKV) %>%
  ggplot(aes(x_cell, y_cell, fill = tot_CHIKV + 1)) +
  geom_point_trace(
    size = 1,
    stroke = 0.5
  ) +
...
xen_dat %>%
  filter(fov == "Group_D_6") %>%
  arrange(Ag_tag) %>%
  ggplot(aes(x_cell, y_cell, fill = Ag_tag + 1)) +
  geom_point_trace(
    size = 1,
    stroke = 0.5
  ) +
