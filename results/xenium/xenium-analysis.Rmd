...
params:
  ...
  xen_ref:     "~/Dropbox/Ryan/Projects/antigen-exchange/results/sobjs/2024-07-01"
  so_dir:      "~/Dropbox/Ryan/Projects/antigen-exchange/results/xenium/2024-03-22"
  ref_dir:     "ref"
  clst_rsln:   10
  slide_id:    1
  ...
  # Sample identify with 6wk and 3wk barcode identity (listed in this order)
...
# Identify Ag+ cells
# * set Ag tag signals for each sample based on params$samples
ag_feats <- str_c("xenium_custom_", ag_targets)

xen <- xen %>%
  imap(~ {
    ag_info <- params$samples[[.y]]
...
```{r "cell type reference", eval = create_object}
# Create reference using scRNA-seq data
ref_so <- c("neg", "pos") %>%
  map(~ {
    file.path(params$xen_ref, str_c("cd45", .x, "_so.qs")) %>%
      qread() %>%
      subset(mouse == "6wk")
  })
...
```{r "OLD refine cell types", include = FALSE, eval = FALSE}
...
bar_dat %>%
  ...
  mutate(mouse = str_c(mouse, "-", fov)) %>%
  
  ggplot(aes(mouse, value, fill = name)) +
