...
dist_meta <- dists %>%
  ...
  slice(1) %>%
  ungroup() %>%
  dplyr::select(other, lec_id, lec_dist) %>%
...
Ag+ LECs and DCs are shown for all DCs for each tissue section.
Lines (bottom) connect the nearest Ag+ LEC identified for each DC.

```{r "Ag+ LECs/DCs 1", fig.width = 8, fig.height = 10, out.width = "50%", results = "asis"}
# Format LEC/DC data
plts <- fovs %>%
  set_names() %>%
  map(~ {
