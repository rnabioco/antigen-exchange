...
.plot_full_fov <- function(df_in, fov_in, color, trace = list(), clrs,
                           size = 0.5, trace_size = 2, trace_clr = "black") {
  # Format input data
...
* Further refinement of cell types
* Analysis of slide 2
...
Cells were downsampled to allow for visualization of all cell types.
LN sections showing all cells are included at the bottom.
...
small <- small %>%
  mutate_meta(
    ...
    lec_Ag_type = fct_relevel(lec_Ag_type, levels(ag_dat$lec_type))
  )
...
```{r "Ag+ LECs and DCs", fig.width = 13, fig.height = 7}
...
Ag+ LECs and DCs are shown for tissue section D. Lines (bottom) connect the nearest
Ag+ LEC identified for each DC.

```{r "FOV D Ag+ LECs/DCs", fig.width = 13, fig.height = 5.5}
...
```{r "FOV D Ag+ LECs/DCs w/lines", fig.width = 13, fig.height = 5.5}
# Plot Ag+ LECs and DCs
plts <- dcs %>%
  map(~ {
    typ <- .x
...
hist <- hist_dat %>%
  ...
  scale_fill_manual(values = c(`Ag-low` = "grey85", `Ag-high` = "#D7301F")) +
  scale_color_manual(values = c(`Ag-low` = "black", `Ag-high` = "#D7301F")) +
  scale_alpha_manual(values = c(`Ag-low` = 0.5, `Ag-high` = 0.85)) +
  scale_x_log10() +
  base_theme +
  theme(
    aspect.ratio    = 0.5,
...
DC subsets. Vertical lines indicate the median distance.

```{r "DC proximity histograms"}
# Plot distance from nearest Ag+ LEC
dc_hist <- hist_dat %>%
...
```{r "DC proximity sections", fig.width = 14, fig.height = 7}
# Scatter plot with points colored by distance from Ag+ LEC
prox_plts <- dcs %>%
  map(~ {
    typ <- .x
    
    dist_plts <- c("Ag-low", "Ag-high") %>%
      map(~ {
        cls <- .x
