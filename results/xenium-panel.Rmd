...
```{r "cell type markers"}
# Marco LECs do not look accurate for some Ag datasets
# see expression of B cell markers
...
# top_marks <- markers %>%
#   arrange(object, clmn, group, desc(logFC), padj) %>% 
#   filter(
#     clmn == "subtype",
#     !grepl("^H2-", feature)
#   ) %>%
...
plt_dat %>%
  ...
  map(~ {
    ...
    #   tidyr::expand(sample, subtype, gene) %>%
    #   mutate(annotation = gn_ann[gene])
    
    dat %>%
      # full_join(typs, by = c("sample", "subtype", "gene", "annotation")) %>%
...
gns <- c(
  ...
  "Siglech", # Siglec-H DCs

  "Ccr7"     # CCR7hi DCs
)

top_marks %>%
  filter(feature %in% gns) %>%
  as.data.frame()

# Ag data
# plt_dat <- lec_so %>%
plt_dat <- dc_so %>%
  FetchData(c("sample", "tm", "mouse", "subtype", gns)) %>%
  as_tibble(rownames = ".cell_id") %>%
...
# CHIKV data
plt_dat <- chikv_so %>%
  subset(cell_type_2 == "LEC") %>%
  FetchData(c("treatment", "tm", "rep", "lec_type", gns)) %>%
  mutate(
    mouse   = str_c(treatment, "-", tm, "-", rep),
    subtype = lec_type
  ) %>%
...
# Create heatmaps
plt_dat %>%
  ggplot(aes(subtype, name, fill = value)) +
  geom_tile() +
  facet_wrap(~ mouse) +
