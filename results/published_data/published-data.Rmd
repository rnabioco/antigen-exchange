...
```{r "load models"}
# Load RF models
ag_mods  <- qread(here(params$mod_dir, "ag-high_best_models.qs"))
ag_feats <- qread(here(params$mod_dir, "ag-high_features.qs"))

all_ag_feats <- ag_feats %>%
...
```{r "load abe data"}
so_file  <- here(params$so_dir, "abe.qs")
...
# MISSING 28/122 FEATURES
rf_feats <- unlist(ag_feats$cLEC, use.names = FALSE)

hu_feats <- hlogs %>%
  filter(
    gene_mm %in% rf_feats,
    gene_hs %in% rownames(hu_so)
  ) %>%
...
Data from [Lei et al.](https://doi.org/10.1084/jem.20221847) were downloaded
and processed.

UMAP projections show cell type annotations, LECs and BECs are circled.

```{r "cell types", fig.width = 8, fig.height = 6}
# Plot colors
clrs <- unique(lei_so$cell_type)

clrs <- set_names(
  as.character(met.brewer("Austria", n_distinct(clrs))),
  clrs
)

clrs["unassigned"] <- "grey65"

# Create UMAPs
top <- list(Inf, c("LEC", "BEC"))

circ_dat <- lei_so@meta.data %>%
  filter(cell_type %in% c("LEC", "BEC")) %>%
  summarize(across(starts_with("UMAP_"), mean), .groups = "drop")
...
Expression of key cell type markers is shown below.

```{r "type markers", fig.width = 11, fig.height = 8}
gns <- c(
  "Ptprc",
  "Cd3d",  "Cd19",
  "Pdpn",  "Pecam1",
...
Data from [Takeda et al.](https://doi.org/10.1016/j.immuni.2019.06.027) were
downloaded and processed.
This study includes scRNA-seq data for 4 axillary lymph nodes (Ax LN) and 2
head and neck lymph nodes (HN LN).
Ag-competent cells were predicted for human cLECs (LEC1) using a random forest
model trained using mouse day 14 cLECs.

The fraction of predicted Ag-competent cells is shown below for each sample.

```{r "takeda colors"}
# Cell type colors
typs <- unique(hu_so$type)
...
hu_so@meta.data %>%
  ...
  ggplot(aes(sample, frac_high)) +
  geom_col(color = "black", fill = ag_clrs[[2]]) +
...
# # Calculate mean expression of Ag-high module
# # BECs also show similar proportion of Ag-high cells as LEC1
# # while BECs show lower overall expression of the Ag-high cLEC module,
# #   they also show lower overall expression of the Ag-low cLEC module
# gns <- hu_modules$low
# 
# mean_expr <- hu_so %>%
#   FetchData(c("sample", "type", "pred", gns)) %>%
#   as_tibble(rownames = "cell_id") %>%
#   
#   pivot_longer(all_of(gns)) %>%
#   group_by(cell_id) %>%
#   summarize(mean_module_expression = mean(value), .groups = "drop") %>%
#   column_to_rownames("cell_id")
# 
# hu_so <- hu_so %>%
#   AddMetaData(mean_expr)
# 
# hu_so@meta.data %>%
#   filter(type %in% c("BEC", "LEC1")) %>%
#   ggplot(aes(sample, mean_module_expression, fill = pred)) +
#   geom_boxplot() +
#   facet_wrap(~ type) +
#   djvdj_theme()
```
...
```{r "Ag modules", fig.width = 8, fig.height = 3}
# Plot Ag-low/high gene modules
hu_so@meta.data %>%
  filter(type == "LEC1") %>%
  mutate(pred = fct_relevel(pred, names(ag_clrs))) %>%
  pivot_longer(all_of(names(module_nms))) %>%
  ggplot(aes(sample, value, fill = pred)) +
  geom_boxplot(outlier.size = 0.2, notch = TRUE) +
  facet_wrap(
    ~ name, nrow = 1,
...
hu_so %>%
  mutate_meta(
    mutate,
    pred = ifelse(type != "LEC1", as.character(NA), as.character(pred))
  ) %>%
  plot_scatter(
    "pred", group_col = "sample",
    size        = 0.1,
...
ACKR4 expression is shown below for each cell type identified for each sample.

```{r "ACKR4 expression", fig.width = 8, fig.height = 6}
hu_so %>%
  plot_violin(
    "ACKR4", "type",
    group_col    = "sample",
