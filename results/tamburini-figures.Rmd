...
subset for each sample into two groups based on Ag-score. The cutoffs
identified for the d14 dataset were then used to identify Ag-high cells for the
other timepoints. With this approach there is a different cutoff for each
LEC subset, but the cutoff is consistent across timepoints.
...
```{r "extended scatter plots", fig.width = 14, fig.height = 4}
# Plot data
lec_lvls <- c("cLEC", "fLEC", "Collecting")
...
```{r "AG-EX DOUBLE POSITIVE EXTENDED", fig.width = 14, fig.height = 3}
# Format plot data
lvls <- c("Ag-low", "Ag-high", "single-high", "double-high")
...
```{r "GEOMX HEATMAP", fig.width = 5, fig.height = 5}
 ht
```
