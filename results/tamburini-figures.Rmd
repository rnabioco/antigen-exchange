...
lvls <- c("Ag-low", "single-high", "double-high")

clmns <- c(
  "subtype", "mouse", "vaccination",
  "Ag_class_dual", "exp", ag_modules
)
...
p_dat <- plt_dat %>%
  ...
  rowwise() %>%
  mutate(padj = .format_pvalue(padj)) %>%
  ungroup()
...
rep_ex_bxs <- ex_bx_args %>%
  pmap(~ {
    ...
    objs <- purrr::discard(bx_args$exps, is.null)
...
```{r "Fig S3", fig.width = 10, fig.height = 12}
# Create final figure
figS3 <- plot_grid(
  plotlist = append(rep_ex_bxs, list(ex_scat$`exp-2`)),
  labels = c("A", "B", "C", "D"),
  label_size = ttl_pt2 * 1.5,
