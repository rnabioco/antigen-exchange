...
create_module_scatter <- function(df_in, ag_clmns, module_clmns, module_title,
  ...
  lab_dat <- dat %>%
    ...
    summarize(
      s_res = list(cor.test(!!sym(module_title), `Ag-score`, method = "spearman")),
      s     = map_dbl(s_res, ~ .x$estimate[[1]]),
      s_p   = map_dbl(s_res, ~ .x$p.value),
      p_res = list(cor.test(!!sym(module_title), `Ag-score`, method = "pearson")),
      p     = map_dbl(p_res, ~ .x$estimate[[1]]),
      p_p   = map_dbl(p_res, ~ .x$p.value),
      n     = n_distinct(.cell_id),  # tricky to include commas with plotmath
      .groups = "drop"
    ) %>%
    mutate(p_adj = p.adjust(s_p, method = "BH")) %>%
    rowwise() %>%
    mutate(
      r_lab = str_c("italic(r[s]) == ", round(s, digits = 2)),
