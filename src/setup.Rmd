...
typ_clrs[c("LEC", "DC", "Fibroblasts")] <- c("#CD3D30", "#56B4E9", "#BF73A6")

typ_clrs["unassigned"] <- other_clr

typ_clrs <- typ_clrs %>%
  spruce_colors(filter = "colorblind", difference = 15)
...
lec_clrs["unassigned"] <- other_clr

dc_clrs <- dc_so@meta.data %>%
  pull(subtype) %>%
  unique()

dc_clrs <- clrs[!clrs %in% lec_clrs] %>%
