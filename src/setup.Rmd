...
# # Plot Ag-low/high grous
# x %>%
#   filter(Ag_score > 0) %>%
#   ggplot(aes(Ag_score, ..count.. + 1, fill = Ag_class)) +
#   geom_histogram() +
#   facet_grid(mouse ~ subtype) +
#   djvdj_theme() +
#   scale_y_log10()
```

```{r "Ag-high 2", eval = FALSE}
...
ag_threshold <- clust_dat %>%
  mutate(km_class = kmeans(Ag_score, centers = 3)$cluster) %>%
  group_by(km_class) %>%
  summarize(n = n(), Ag_score = min(Ag_score)) %>%
  arrange(desc(Ag_score)) %>%
  mutate(km_class = row_number())

ag_threshold <- set_names(ag_threshold$Ag_score, ag_threshold$km_class)

# Set Ag-high/low classifications
all_meta <- all_meta %>%
  mutate(
    Ag_group = case_when(
      Ag_score >= ag_threshold[1] ~ 1,
      Ag_score >= ag_threshold[2] ~ 2,
      Ag_score >= ag_threshold[3] ~ 3,
      TRUE                        ~ 4
    )
  )
