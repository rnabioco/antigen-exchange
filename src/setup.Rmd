...
#' 
#' Used for figure 3 
create_exchange_scatter <- function(df_in, x = c(Ag_3wk_score = "Ag-score (21 day)"),
  ...
  lab_dat <- df_in %>%
    ...
    summarize(
      cor_res = list(cor.test(!!sym(x_var), !!sym(y_var), method = "pearson")),
...
#' 
#' Used for figure 3
create_module_boxes <- function(df_in, module_clmns, module_title, lvls,
                                p_alt = "two.sided") {
...
create_module_scatter <- function(df_in, ag_clmns, module_clmns, module_title,
                                  clrs, label_x = -Inf,
                                  label_hjust = -0.1, label_start = -0.5,
                                  label_pad = 0.15) {
  ...
  dat <- df_in %>%
    ...
    filter(subtype == str_remove(module_name, "_[^_]+$")) %>%
    mutate(
      subtype       = fct_relevel(subtype, typs),
      Ag_class_dual = fct_relevel(Ag_class_dual, rev(names(clrs)))
    )
  ...
  lab_dat <- dat %>%
    ...
    summarize(
      cor_res = list(cor.test(!!sym(module_title), `Ag-score`, method = "pearson")),
      cor     = map_dbl(cor_res, ~ .x$estimate[[1]]),
      p_val   = map_dbl(cor_res, ~ .x$p.value),
  ...
  res <- dat %>%
    ggplot(aes(!!sym(module_title), `Ag-score`, color = Ag_class_dual)) +
    geom_point(size = 0.7) +
    geom_smooth(
...
sobjs %>%
  map_dfr(~ {
    .x@meta.data %>%
      ...
      group_by(directory, exp, sample, tm, mouse, vaccination, cell_sort) %>%
