...
```{r "epithelial cells", eval = create_sobjs}
...
# * FRC object will not include updated annotations, but these subsets are not
...
set_clrs <- set_names(
  c("#E7301F", "#56B4E9", "#BF73A6", "#0072B2", "#F0E442", other_clr),
  set_typs
)
...
other_clrs <- typ_clrs[!typ_clrs %in% lec_clrs]

other_clrs <- other_clrs %>%
  assign_colors(lec_typs[!lec_typs %in% names(lec_clrs)])

lec_clrs <- c(lec_clrs, other_clrs)
...
typ_clrs <- set_clrs <- clrs %>%
  assign_colors(typs, filter = "deutan")

# typ_clrs["unnassigned"] <- other_clr
...
lec_clrs[set_typs] <- c(
  "#E69F00", "#56B4E9", "#F0E442",
  "#E7301F", "#000000", "#0072B2",  # typ_clrs[["BEC"]],
  other_clr
)
