...
#' e.g. total counts expressed in millions. If NULL signal is not normalized
...
calc_ag_score <- function(obj, clmns, control_types = c("B cells", "T cells"),
                          ...
                          operation = `-`, slot = "counts", final_trans = log1p) {
  ...
  # Subtract background signal and transform
  res <- res %>%
    mutate_meta(~ {
      .x %>%
        mutate(across(
          ...
          ~ {
            ...
            if (!is.null(control_types)) {
              ...
              res   <- operation(.x, t_cut)         # correct for background
    ...
    imap(~ {
      ...
      if (show_scale_title) {
        u <- u +
          ...
          theme(legend.title = element_text(size = ttl_pt2, hjust = 0.5))
      }
...
#' 
#' Used for figure 5
create_chikv_class_bars <- function(df_in, ttl) {
...
#' 
#' Used for figure 5
create_chikv_module_boxes <- function(df_in, module_clmns, clrs = treat_clrs,
                                      ttl, n_p = NULL) {
  
  # Format data for plotting
  treats <- unique(df_in$treatment)
  ...
  dat <- df_in %>%
    ...
    mutate(
      ...
      # module_subtype = fct_relevel(module_subtype, rf_typs),
      # module_type    = fct_relevel(module_type, c("Ag-high", "Ag-low"))
    ) %>%
  ...
  p_dat <- dat %>%
    ...
    summarize(
      p = list(wilcox.test(value[treatment == treats[1]], value[treatment == treats[2]])),
      p = map_dbl(p, ~ .x$p.value),
      .groups = "drop"
    ) %>%
    mutate(
      p_adj = p.adjust(p, method = "bonferroni", n = n_p %||% length(p))
    ) %>%
    rowwise() %>%
  ...
  res <- dat %>%
    ...
    facet_grid(module_type ~ subtype, scales = "free_y") +
    scale_fill_manual(values = clrs) +
    scale_alpha_manual(values = rep(1, n_distinct(df_in$rep))) +
...
# normalizing data
norm_file <- here(params$ref_dir, "norm_factors.tsv")
...
if (!file.exists(norm_file)) {
  ...
  norm_factors <- sam_info %>%
    mutate(
      raw_lib_size = future_pmap_dbl(., ~ {
        args <- list(...)
        .calc_lib_size(
          here(params$res_dir[[args$experiment]], args$directory),
          "raw_feature_bc_matrix"
        )
      }),
      filt_lib_size = future_pmap_dbl(., ~ {
        args <- list(...)
        .calc_lib_size(
          here(params$res_dir[[args$experiment]], args$directory),
...
norm_factors <- read_tsv(norm_file)

norm_factors <- set_names(
  norm_factors$raw_lib_size,
...
sobjs %>%
  map_dfr(~ {
    .x@meta.data %>%
      ...
      group_by(directory, exp, sample, tm, mouse, vaccination, cell_sort, qc_class) %>%
...
# 2. Check unassigned cells, some may be epithelial cells. They do express
#    epithelial cell markers including Cdh1 and Krt8
...
# NEED TO:
# 1. check cDC2s, there is a cluster that might be T-bet+. The cDC2 subset
#    shows higher expression of T-bet+ markers than the T-bet- cells, however,
#    the expression of T-bet+ markers is still very sparse and not especially
#    convincing
...
```{r "save suerat objects", eval = create_sobjs}
...
lec_clrs["fLEC"]     <- "#E69F00"
lec_clrs["Ptx3_LEC"] <- "#F0E442"
