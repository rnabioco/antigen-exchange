...
#' 
#' Used for figure 5
create_chikv_class_bars <- function(df_in, p_data, ttl) {
  
  p_data <- p_data %>%
    filter(subtype %in% df_in$subtype)
  
  res <- df_in %>%
    ggplot(aes(treatment, frac_comp, fill = treatment, alpha = rep)) +
...
create_chikv_module_boxes <- function(df_in, module_clmns, clrs = treat_clrs,
  ...
  p_dat <- dat %>%
    ...
    summarize(
      p = wilcox.test(
        value[treatment == treats[1]],
        value[treatment == treats[2]]
      )$p.value,
      .groups = "drop"
    ) %>%
    mutate(
      p_adj = p.adjust(p, method = "fdr", n = n_p %||% length(p))
    ) %>%
    rowwise() %>%
