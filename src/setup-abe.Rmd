...
get_abe_preds <- function(so_in, rf_type, hlogs, rf_feats = rf_feats,
                          rf_mods = rf_mods_best) {
  # Pull data for RF features
  # MISSING 10/74 FEATURES
  abe_feats <- c(rf_feats[[rf_type]], recursive = TRUE, use.names = FALSE)
  
  hu_feats <- hlogs %>%
    filter(
      gene_mm %in% abe_feats,
      gene_hs %in% rownames(so_in)
    ) %>%
  ...
  cli_alert_info(
    "{length(missing_feats)} / {length(abe_feats)}
     ({round(length(missing_feats)/length(abe_feats)*100, 1)}%) genes missing"
  )
