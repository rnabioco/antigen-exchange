...
# Custom targets
ag_targets <- unname(params$xen_ag_targets)
ag_clmns   <- names(params$xen_ag_targets)
...
xen %>%
  qsave(here(params$xen_dir, str_c("xen", sffx)))
```
...
# Save objects
lec %>%
  qsave(here(params$xen_dir, str_c("lec", sffx)))

frc %>%
  qsave(here(params$xen_dir, str_c("frc", sffx)))
```
...
small %>%
  qsave(here(params$xen_dir, str_c("small", sffx)))
```

```{r "{{.x}} xen load objects", eval = FALSE}
# Load objects
...
```{r "{{.x}} xen format data", eval = FALSE}
