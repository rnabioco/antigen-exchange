...
```{r "MORE TESTING", eval = FALSE}
typ <- "cLEC"

mod <- rf_mods_best %>%
  filter(subtype == typ)

train_bcs <- mod$training_bcs[[1]]

dat <- lec_so %>%
  subset(subtype == typ & mouse == "d14") %>%
  FetchData(c("Ag_class_ml", rf_feats[[typ]], recursive = TRUE, use.names = FALSE)) %>%
  rename_with(~ str_replace_all(.x, "-", "_")) %>%
  rename_with(~ str_replace_all(.x, "\\(|\\)", "_")) %>%
  rename_with(~ str_c("x", .x), matches("^[0-9]"))

dat <- dat[!rownames(dat) %in% train_bcs, ] %>%
  rownames_to_column(".cell_id") %>%
  group_by(Ag_class_ml) %>%
  slice_sample(n = 139, replace = FALSE) %>%
  ungroup() %>%
  column_to_rownames(".cell_id")
