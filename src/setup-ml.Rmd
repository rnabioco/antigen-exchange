...
create_gn_plots <- function(so_in, gns, plt_clrs, p_alt = "greater", n_gns = 10,
  ...
  gn_dat <- so_in %>%
    FetchData(c("mouse", "tm", "subtype", "pred_grp", "training_data", gns)) %>%
    as_tibble(rownames = ".cell_id") %>%
  ...
  plt_dat <- gn_dat %>%
    group_by(name, mouse, tm, subtype, pred_grp) %>%
    summarize(
      med = median(value),
  ...
  p_dat <- gn_dat %>%
    group_by(mouse, tm, subtype, pred_grp, name) %>%
    mutate(y = quantile(value, 0.75)) %>%
    group_by(mouse, tm, subtype, name) %>%
    summarize(
      y = max(y),
  ...
  res <- plt_dat %>%
    ...
    ggplot(aes(pred_grp, med, color = tm)) +
    geom_segment(
  ...
  res <- res +
    ...
    facet_grid(name ~ tm, scales = "free_y", switch = "y") +
...
# Save LEC object with additional RF results
