...
train_rf <- function(df_in, data_clmn, param_lst, train_fn = ranger,
  ...
  res <- pmap_dfr(rf_params, ~ {
    ...
    caret_control <- trainControl(
      ...
      number          = 10,
      seeds           = as.list(1:11),
      summaryFunction = calc_training_stats
    )
    ...
    caret_args$num.threads <- caret_args$num.threads %||% 12
    caret_args$verbose     <- caret_args$verbose %||% FALSE

    # Train initial model
    # pull out key features based on p-value
    set.seed(train_seed)
    
    raw_mod   <- mod <- lift_dl(train)(caret_args)
    raw_f1    <- raw_mod$results$F1
    raw_kappa <- raw_mod$results$Kappa
    
    key_feats <- importance_pvalues(raw_mod$finalModel)
    ...
    if (select_feats) {
      ...
      grid_params$mtry    <- min(grid_params$mtry, n_feats)
      caret_args$tuneGrid <- grid_params
    ...
    if (!is.null(df_list)) {
      ...
      frac_stats <- df_list %>%
        ...
        mutate(
          frac_actual_high = sum(rf_dat[[data_clmn]] == frac_grp) / nrow(rf_dat)
        )
...
rf_stats <- rf_typs %>%
  map_dfr(~ {
    ...
    param_lst <- list(
      ...
      low           = c(seq(1, 5, 1), NA),  # include NAs so original class
      high          = c(seq(1, 5, 1), NA),  # distributions are also tested
      num.threads = 14
    )
