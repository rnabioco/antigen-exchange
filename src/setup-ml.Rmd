...
rf_predict <- function(mod, dat, data_clmn = NULL, return_stats = TRUE,
  ...
  res <- res %>%
    ...
    mutate(
      correct = as.character(pred) == as.character(!!sym(data_clmn))
    ) %>%
    group_by(pred) %>%
...
train_rf <- function(df_in, data_clmn, param_lst,
                     feat_p = 0.05, min_feats = 1, min_cells = 30,
                     select_feats = TRUE, df_list = NULL,
  ...
  res <- pmap_dfr(rf_params, ~ {
    ...
    if (select_feats) {
      ...
      feats_p   <- key_feats[key_feats < feat_p]
      key_feats <- names(feats_p)
...
```{r "RF identify Ag DEGs", eval = run_rf}
...
# Train top models for each cell type
# * filter based on Balanced Accuracy and F1
rf_mods_best <- rf_stats %>%
  distinct(high / low, F1, `Balanced Accuracy`, .keep_all = TRUE) %>%
  group_by(subtype) %>%
  slice_max(F1, prop = 0.2) %>%
...
rf_mods_best <- rf_mods_best %>%
  pmap_dfr(~ {
    ...
    rf_dat <- lec_so %>%
      subset(subtype == typ & mouse == rf_mouse) %>%
      FetchData(unique(c(rf_dat_clmn, feats)))
...
```{r "RF predict Ag class", eval = run_rf}
# Predict Ag class
...
rf_preds <- rf_mods_best %>%
  pmap_dfr(~ {
    ...
    dat %>%
      ...
      mutate(training_data = rownames(.) %in% args$training_bcs)
  })
...
```{r "RF load CHIKV data", eval = run_rf}
# Load CHIKV data
...
chikv_dat <- chikv_so %>%
  FetchData(c(chikv_clmns, all_feats))

# Add zeros for missing genes
# * to predict Ag classes, need to fix gene names to conform to required
#   feature format
...
```{r "RF predict CHIKV", eval = run_rf}
...
```{r "RF PERFORMANCE USING CHIKV DATA", eval = FALSE}
...
rf_mods <- rf_mods %>%
  pmap_dfr(~ {
    ...
    chikv_so <- chikv_so %>%
      subset(!!sym(chikv_type_clmn) == args$subtype) %>%
...
```{r "OLD MODELS TEST", eval = FALSE}
rf_stats <- read_tsv(here(params$mod_dir, "ag-high_stats.tsv.gz"))
x <- qread(here(params$mod_dir, "ag-high_models_new.qs"))
...
```{r "OLD MODELS TEST 2", eval = FALSE}
# Train top models
x <- rf_mods_best %>%
  pmap_dfr(~ {
    args <- list(...)
  ...
  imap_dfr(~ {
    ...
    .x %>%
      pmap_dfr(~ {
        ...
        stats <- stats[stats %in% names(args)]

        rf_dat %>%
          train_rf(
            data_clmn       = rf_dat_clmn,
            param_lst       = append(args[stats], list(splitrule = "gini")),
            include_objects = TRUE
          ) %>%
          mutate(subtype = args$subtype)
      })
...
```{r "UNUSED RF FUNCTIONS", eval = FALSE}
# These are updated functions that were not used for final models
...
#' rows
#' @param data_clmn column in df_in containing response variable
#' @param param_lst named list of RF parameters to expand and test,
#' classes present in data_clmn can be downsampled at different ratios by
#' including the class names and weights (an integer) in param_lst,
#' specify NA for one of the classes to skip downsampling and use the data as is
#' @param feat_p p-value cutoff for selecting best features
#' @param min_feats minimum number of features to allow after feature selection
#' @param min_cells minimum number of cells required for each group in data_clmn
#' @param select_feats should features be filtered based on p-value
...
train_rf <- function(df_in, data_clmn, param_lst,
  ...
  res <- pmap_dfr(rf_params, ~ {
    ...
    if (args$mtry > (ncol(rf_train) - 1)) {
      cli_warn("mtry ({args$mtry}) is greater than the number of features")
...
```{r "UNUSED TRAIN MODELS", eval = FALSE}
# Test parameters and calculate stats for each LEC subtype
rf_stats <- rf_typs %>%
  map_dfr(~ {
    feats <- degs %>%
...
```{r "UNUSED ASSESS WITH CHIKV DATA", eval = FALSE}
# Format CHIKV RF data
...
new_degs_2 <- degs %>%
  filter(padj < 0.05, logFC > fc, subtype == "cLEC", mouse == "d14") %>%
  as_tibble()
